...
// Уникальный ID для этого экземпляра скрипта
...
function debugLog(message) {
    if (config.debug) {
        ...
        console.log(`[${currentTime}] [DEBUG][${config.accountInfo.nickname || 'Unknown'}][${instanceId.substr(0, 8)}]`, message);
    }
}
...
// Новая функция для очистки старых блокировок
function cleanupOldLocks() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && (key.startsWith('tg_update_lock_') || key.startsWith('tg_update_processed_'))) {
                        try {
                                const parts = key.split('_');
                                const updateId = parseInt(parts[parts.length - 1]);
                                // Если обновление старше 5 минут, удаляем
                                if (now - updateId > maxAge) {
                        ...
                        if (xhr.status === 200) {
                                try {
                                        ...
                                        if (data.ok && data.result.length > 0) {
                                                const currentSharedId = getSharedLastUpdateId();
                                                const newUpdates = data.result.filter(u => u.update_id > currentSharedId);
...
function processUpdates(updates) {
        for (const update of updates) {
                // Создаем уникальный ключ для этого обновления
                const updateLockKey = `tg_update_lock_${update.update_id}`;
                const updateProcessedKey = `tg_update_processed_${update.update_id}`;
                
                // Проверяем, не обработано ли уже это обновление
                if (localStorage.getItem(updateProcessedKey)) {
                        debugLog(`Обновление ${update.update_id} уже обработано, пропускаем`);
                        continue;
                }

                // Пытаемся захватить блокировку
                const lockValue = localStorage.getItem(updateLockKey);
                if (lockValue && lockValue !== instanceId) {
                        debugLog(`Обновление ${update.update_id} обрабатывается другим экземпляром (${lockValue.substr(0, 8)}), пропускаем`);
                        continue;
                }

                // Устанавливаем блокировку
                localStorage.setItem(updateLockKey, instanceId);
                
                // Двойная проверка после небольшой задержки (для предотвращения race condition)
                const checkDelay = Math.random() * 50; // 0-50ms
                setTimeout(() => {
                        if (localStorage.getItem(updateLockKey) !== instanceId) {
                                debugLog(`Потеряли блокировку для ${update.update_id}, пропускаем`);
                                return;
                        }
                        ...
                        localStorage.setItem(updateProcessedKey, instanceId);
                        
                        // Удаляем блокировку
                        localStorage.removeItem(updateLockKey);
                        
                        // Очищаем старые записи (старше 5 минут)
                        cleanupOldLocks();

                        // Обрабатываем обновление
                        processUpdateContent(update);
                }, checkDelay);
        }
}

function processUpdateContent(update) {
        if (update.message) {
                ...
                if (update.message.reply_to_message) {
                        ...
                        if (replyToText.includes(`✉️ Введите ник аккаунта для активации AFK режима:`)) {
                                ...
                                } else if (accountNickname) {
                                        debugLog(`Сообщение для аккаунта ${accountNickname}, текущий: ${config.accountInfo.nickname}, игнорируем`);
                                }
                                return;
                        }
...
debugLog(`Скрипт запущен с уникальным ID: ${instanceId}`);
