...
# Подпапка с лёгкими загрузчиками (вместо тяжёлых оригиналов) —
# именно отсюда теперь реально качаются файлы кастомных интерфейсов
# в _deploy_custom_ui_files(). CUSTOM_UI_URL/IntLoad.js по-прежнему
# в родительской папке — это просто манифест, ему loader-обвязка не нужна.
CUSTOM_UI_LOADERS_URL = CUSTOM_UI_URL + "/%D0%97%D0%B0%D0%B3%D1%80%D1%83%D0%B7%D1%87%D0%B8%D0%BA%D0%B8"
...
class InstallerAPI:
    ...
    def _deploy_custom_ui_files(self, ifaces: list):
        ...
        Качаем из CUSTOM_UI_LOADERS_URL (подпапка "Загрузчики"), а НЕ из
        CUSTOM_UI_URL — там лежат тяжёлые оригиналы, которые сами тянут
        актуальный код с GitHub в рантайме (см. zkm.js / MvdMenu.js /
        ZkmScreenNotification.js — каждый из них при загрузке делает XHR
        обратно в CUSTOM_UI_URL за реальным содержимым). В assets/ должны
        попадать именно лёгкие загрузчики, иначе обновления интерфейсов
        перестанут подхватываться без переустановки клиента."""
        if not self.radmir_path:
            return
        ...
        for filename in all_files:
            url = f"{CUSTOM_UI_LOADERS_URL}/{filename}"
            try:
...
def main():
    ...
    except: passimport os, random, string, threading, tempfile, requests, json
from pathlib import Path
import webview
