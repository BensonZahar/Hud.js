...
(function(){
console.log("[HAS-mvdF] патч запускается (адаптирован для eval)");

// ═══════════════════════════════════════════════════════════════════════════════
// АДАПТАЦИЯ ДЛЯ mvdF.js (выполнение через eval)
// ═══════════════════════════════════════════════════════════════════════════════
// При выполнении через eval():
// 1. Mu недоступна напрямую (экспортируется из Hud.js)
// 2. Oe/Ao/sr недоступны (объявлены через const в index.js)
//
// Решение:
// - Получаем компонент через import("./Hud.js")
// - Получаем хелперы через window.App._context или через сам компонент
// ═══════════════════════════════════════════════════════════════════════════════
...
function __hasFindVueHelpersFromApp() {
    ...
    try {
        if (window.App && window.App._context) {
            ...
            // Пытаемся получить хелперы из контекста приложения
...
function __hasPatchHudComponent(__hasComp) {
    ...
    if (__hasComp.components && __hasComp.components.HudHassle && __hasComp.components.HudRadmir) {
        ...
        var __hasHelpers = __hasFindVueHelpersFromApp();
...
// ═══════════════════════════════════════════════════════════════════════════════
// ТОЧКА ВХОД: Получаем компонент через import()
// ═══════════════════════════════════════════════════════════════════════════════
(function __hasInitCompPatch() {
    console.log("[HAS-mvdF] Загружаю компонент Hud.js через import()...");
    
    import("./Hud.js").then(function(mod) {
        var loadedComp = mod && mod.default;
        if (__hasLooksLikeHudComp(loadedComp)) {
            console.log("[HAS-mvdF] ✅ Компонент Hud получен, применяю патч...");
            __hasPatchHudComponent(loadedComp);
        } else {
            console.warn("[HAS-mvdF] ❌ Не удалось получить компонент Hud из чанка Hud.js", mod);
        }
    }).catch(function(err) {
        console.warn("[HAS-mvdF] ❌ Не удалось подгрузить чанк Hud.js", err);
    });
})();
