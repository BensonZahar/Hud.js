/* ===== /has, /has_s команды + Hassle HUD (Vue3 Hack + Map Fix) ===== */
(function(){
console.log("[HAS] Скрипт инициализирован (Метод: Переопределение ctx + Фикс карты)");
...
var allowed = !!nick&&ALLOWED_NICKS.indexOf(nick)!==-1;
console.log("[HAS] Проверка ника:", nick, "| Разрешен:", allowed);
return allowed;
}
...
infoRight:-1.82, infoTop:-1, infoScale:60,
border:"default"
};
...
style.textContent=
 ".__has-chat-hassle-pos .radmir-chat, .__has-chat-hassle-pos .chat{left:var(--has-chat-left)!important;top:var(--has-chat-top)!important;width:var(--has-chat-width)!important;} "+
 ".__has-chat-hassle-pos .radmir-chat__messages, .__has-chat-hassle-pos .chat__messages{width:var(--has-chat-width)!important;height:var(--has-chat-height)!important;} "+
 ".__has-chat-hassle-pos .radmir-chat-input, .__has-chat-hassle-pos .chat-input{width:var(--has-chat-width)!important;} "+
 ...
 // РАДАР (Добавлен фикс opacity и visibility для карты!)
 ".__has-chat-hassle-pos .hud-hassle-radar, .__has-chat-hassle-pos .hud-radmir-radar{left:var(--has-radar-left)!important;top:var(--has-radar-top)!important;} "+
 ".__has-chat-hassle-pos .hud-hassle-radar__map, .__has-chat-hassle-pos .hud-radmir-radar__map{transform:scale(var(--has-radar-scale))!important;opacity:1!important;visibility:visible!important;} "+
 
 // Правый HUD
 ".__has-chat-hassle-pos .hud-hassle-info, .__has-chat-hassle-pos .hud-radmir-info{right:var(--has-info-right)!important;top:var(--has-info-top)!important;transform:scale(var(--has-info-scale))!important;} "+
 ...
 // СКРЫВАЕМ МОБИЛЬНЫЕ КОНТРОЛЫ HASSLE на ПК
 ".__has-chat-hassle-pos .hud-hassle-controls__joystick, "+
 ".__has-chat-hassle-pos .hud-hassle-controls__pedals, "+
 ".__has-chat-hassle-pos .hud-hassle-controls__buttons, "+
 ".__has-chat-hassle-pos .hud-hassle-controls__threangel, "+
 ".__has-chat-hassle-pos .hud-hassle-controls-right_top, "+
 ".__has-chat-hassle-pos .hud-hassle-controls-right_bottom, "+
 ".__has-chat-hassle-pos .hud-hassle-controls__close, "+
 ".__has-chat-hassle-pos .hud-hassle-speedometer__controls, "+
 ".__has-chat-hassle-pos .hud-hassle-radar .mobile-button, "+
 ".__has-chat-hassle-pos .mobile-button { display:none!important; pointer-events:none!important; } ";
document.head.appendChild(style);
console.log("[HAS] CSS стили инжектированы (с фиксом карты).");
}
...
// ГЛАВНЫЙ ХАК: переопределяем computed isHassleHud на ctx компонента HUD
function __hasForceHudHassle(hud, val) {
    hud.__hassleForced = val;
    
    try {
        if (!hud.$ || !hud.$.ctx) {
            console.warn("[HAS] hud.$.ctx недоступен, хак не применен.");
            return;
        }
        var ctx = hud.$.ctx;
        var root = hud.$.root;
        
        Object.defineProperty(ctx, 'isHassleHud', {
            get: function() { return hud.__hassleForced ? true : (root.engine !== "legacy" && root.isMobile); },
            configurable: true,
            enumerable: true
        });
        
        Object.defineProperty(ctx, 'isMobile', {
            get: function() { return hud.__hassleForced ? true : root.isMobile; },
            configurable: true,
            enumerable: true
        });
        
        Object.defineProperty(ctx, 'engine', {
            get: function() { return hud.__hassleForced ? "hassle" : root.engine; },
            configurable: true,
            enumerable: true
        });

        if (hud.$.accessCache) {
            hud.$.accessCache['isHassleHud'] = 4;
            hud.$.accessCache['isMobile'] = 4;
            hud.$.accessCache['engine'] = 4;
        }
        
        console.log("[HAS] УСПЕХ: Свойства переопределены на ctx");
    } catch(e) {
        console.error("[HAS] ОШИБКА переопределения:", e);
    }
    ...
    setTimeout(function() {
        var hassleEl = document.querySelector('.hud-hassle');
        var radmirEl = document.querySelector('.hud-radmir');
        console.log("[HAS] ПРОВЕРКА DOM -> Hassle:", !!hassleEl, "| Radmir:", !!radmirEl);
        if (val && hassleEl) {
            console.log("[HAS] ✅ Hassle HUD успешно отрендерен!");
        }
    }, 500);
}
...
p.appendChild(__hasSlider("Высота (vh)","chatHeight",10,50,0.1));
...
borderLabel.textContent="Бордер";
...
infoLabel.textContent="Правый HUD";
...
settings=Object.assign({},PC_DEFAULTS,{hassleForced:true});
__hasApplyAll();
__hasRebuildPanel();
});
p.appendChild(pcBtn);
...
settings=Object.assign({},DEFAULTS,{hassleForced:true});
__hasApplyAll();
__hasRebuildPanel();
});
...
if(text==="/has"){
var hud=window.interface && window.interface("Hud");
if(!hud){__hasToast("HUD не инициализирован");return;}
__hasInjectChatStyle();
__hasSetForced(hud,!hud.__hassleForced);
__hasToast(hud.__hassleForced?"HASSLE: включен":"HASSLE: выключен");
return;
} else if(text==="/has_s"){
var hud=window.interface && window.interface("Hud");
if(!hud){__hasToast("HUD не инициализирован");return;}
