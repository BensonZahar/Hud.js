(function(){
  ...
  // ── Логирование ──────────────────────────────────────────────────────
  var HAS_LOG_PREFIX="[HAS-HUD]";
  function __hasLog(){
    var args=Array.prototype.slice.call(arguments);
    args.unshift(HAS_LOG_PREFIX);
  ...
  function __hasLoadSettingsForNick(nick){
    ...
    try{
      ...
      if(!raw){
        ...
        if(legacy){
          try{
            var migrated=Object.assign({},nickDefaults,JSON.parse(legacy));
            __hasLog("__hasLoadSettingsForNick(",nick,"): миграция со старого ключа localStorage, итог=",migrated);
            return migrated;
          }catch(e){__hasWarn("__hasLoadSettingsForNick(",nick,"): ошибка парсинга legacy настроек",e);}
        }
        __hasLog("__hasLoadSettingsForNick(",nick,"): сохранённых настроек нет, беру дефолты=",nickDefaults);
        return nickDefaults;
      }
      var parsed=JSON.parse(raw);
      var result=Object.assign({},nickDefaults,parsed);
      __hasLog("__hasLoadSettingsForNick(",nick,"): загружено из localStorage=",result);
      return result;
    }catch(e){__hasWarn("__hasLoadSettingsForNick(",nick,"): ошибка чтения localStorage",e);return nickDefaults;}
  }
  ...
  function __hasEnsureSettings(){
    ...
    if(nick&&nick!==__hasSettingsNick){
      __hasLog("__hasEnsureSettings(): смена ника",__hasSettingsNick,"->",nick,", перечитываю настройки");
  ...
  function __hasSaveSettings(){
    try{
      ...
      __hasLog("__hasSaveSettings(): сохранено под ключом",key);
    }catch(e){__hasWarn("__hasSaveSettings(): ошибка записи в localStorage",e);}
  }
  ...
  function __hasApplyCSSVars(){
    __hasLog("__hasApplyCSSVars(): применяю позиционирование, settings=",JSON.parse(JSON.stringify(settings)));
  ...
  function __hasApplyToHud(){
    ...
    if(!hud){__hasWarn("__hasApplyToHud(): window.interface(\"Hud\") недоступен, пропускаю");return;}
    ...
    __hasLog("__hasApplyToHud(): chatFontSize=",settings.chatFontSize,"border=",settings.border,
      "isHelloween=",hud.isHelloween,"isNewYear=",hud.isNewYear);
  }

  function __hasApplyAll(){
    __hasLog("__hasApplyAll(): body.hassle=",document.body.classList.contains("hassle"),
      "body.__has-chat-hassle-pos=",document.body.classList.contains("__has-chat-hassle-pos"));
    __hasApplyCSSVars();
    __hasApplyToHud();
  }
  ...
  function __hasSetForced(hud,val,silent){
    ...
    // ВАЖНО: "__has-chat-hassle-pos" — это НАШ собственный класс, он только
    // двигает позицию чата/радара/инфо через CSS-переменные. Сам ВИЗУАЛЬНЫЙ
    // скин HUD/карты (радмировский vs hassle) переключается движком клиента
    // через класс "hassle" на <body> (см. index.js: mounted() у App —
    // this.engine!="legacy" && document.body.classList.add("hassle")).
    // На ПК engine всегда "legacy", поэтому нативный класс сам никогда не
    ...
    if(val){
      ...
      if(window.updatePlayerList){
        __hasLog("вызываю window.updatePlayerList() для мгновенного обновления online/ID");
        window.updatePlayerList();
      }else{
        __hasWarn("window.updatePlayerList недоступен на момент включения HUD");
      }
    }else{
  ...
  window._mvdHassleGetState=function(){
    __hasEnsureSettings();
    var hud=window.interface&&window.interface("Hud");
    var state={ enabled:!!(hud&&hud.__hassleForced), border:settings.border||"default" };
    __hasLog("_mvdHassleGetState() ->",state);
    return state;
  };
  // Явно включить/выключить HUD (val — true/false).
  window._mvdHassleSetEnabled=function(val){
    __hasLog("_mvdHassleSetEnabled(",val,") вызван");
    __hasEnsureSettings();
    var hud=window.interface&&window.interface("Hud");
    if(!hud){__hasWarn("_mvdHassleSetEnabled: window.interface(\"Hud\") недоступен, отмена");return!1;}
  ...
  window._mvdHassleToggle=function(){
    __hasLog("_mvdHassleToggle() вызван");
    __hasEnsureSettings();
    var hud=window.interface&&window.interface("Hud");
    if(!hud){__hasWarn("_mvdHassleToggle: window.interface(\"Hud\") недоступен, отмена");return!1;}
    __hasInjectChatStyle();
  ...
  window._mvdHassleSetBorder=function(border){
    __hasLog("_mvdHassleSetBorder(",border,") вызван");
    __hasEnsureSettings();
  ...
  window._mvdHassleOpenSettings=function(){
    __hasLog("_mvdHassleOpenSettings() вызван");
    __hasEnsureSettings();
    var hud=window.interface&&window.interface("Hud");
    if(!hud){__hasWarn("_mvdHassleOpenSettings: window.interface(\"Hud\") недоступен, отмена");return!1;}
    __hasInjectChatStyle();
    if(!hud.__hassleForced)__hasSetForced(hud,!0,!0);
    __hasShowPanel();
    return!0;
  };
  window._mvdHassleCloseSettings=function(){ __hasLog("_mvdHassleCloseSettings() вызван"); __hasHidePanel(); };
  ...
  (function __hasAutoInit(){
    ...
    __hasLog("__hasAutoInit(): жду появления window.interface(\"Hud\")...");
    var timer=setInterval(function(){
      tries++;
      var hud=window.interface&&window.interface("Hud");
      if(hud){
        clearInterval(timer);
        __hasLog("__hasAutoInit(): Hud найден на попытке",tries);
        __hasEnsureSettings();
        __hasInjectChatStyle();
        if(settings.hassleForced===!0){
          __hasLog("__hasAutoInit(): settings.hassleForced=true, включаю HUD автоматически");
          __hasSetForced(hud,!0,!0);
        }else{
          __hasLog("__hasAutoInit(): settings.hassleForced=false, HUD остаётся выключенным");
        }
      }else if(tries>=maxTries){
        clearInterval(timer);
        __hasWarn("__hasAutoInit(): window.interface(\"Hud\") так и не появился за",maxTries,"попыток");
      }
    },150);
  })();
})();
