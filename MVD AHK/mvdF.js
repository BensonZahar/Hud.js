// === HAS HUD: Vue-патч + настройки/панель + команды /has, /has_s ===
// Всё содержимое has.js объединено здесь, вверху mvdF.js. Оба файла в любом
// случае дописываются в один и тот же index.js (после границы бандла), т.е.
// делят один module-scope — поэтому Vue-хелпер createVNode=d, который нужен
// патчу VoiceChat/HudHassle ниже, доступен отсюда точно так же, как если бы
// код лежал в отдельном has.js. Отдельный has.js больше не нужен.
//
// /has и /has_s зарегистрированы как обычные ветки внутри sendChatInputCustom
// (см. ниже, рядом с /dahk и /console) — так они гарантированно работают
// независимо от того, в каком порядке был бы вставлен код, будь он всё ещё
// раздельным. Автозапуск (принудительное включение Hassle HUD без команды)
// убран — HUD включается только вручную через /has или /has_s.

console.log("[HAS] патч запускается");

// ФИКС ДЛЯ index.js: раньше компонент брался из локальной переменной "Mu",
// которую минификатор оставляет только внутри Hud.js — там это реально
// экспортируемый Hud-компонент. В index.js переменная с таким же именем "Mu"
// существует, но это совершенно другая, внутренняя функция Vue (слияние опций),
// у неё нет .data/.computed/.components, поэтому патч ниже раньше молча проваливался.
// Берём компонент максимально надёжно — через тот же самый динамический
// import("./Hud.js"), которым его лениво грузит сам index.js (см. запись
// Hud:f(()=>d(()=>import("./Hud.js"),...)) в реестре интерфейсов чуть выше по файлу).
// import() резолвится по export-биндингу "default", а не по имени переменной,
// поэтому коллизия имён здесь уже не страшна.
(async function __hasPatchHudComponent(){
var __hasComp=null;
try{
var __hasMod=await import("./Hud.js");
__hasComp=__hasMod&&__hasMod.default;
}catch(err){
console.warn("[HAS] не удалось импортировать ./Hud.js — патч компонента НЕ применён",err);
return;
}
if(!__hasComp){
console.warn("[HAS] import(\"./Hud.js\") вернул модуль без default-экспорта — патч НЕ применён",__hasMod);
return;
}
...
})(); // конец __hasPatchHudComponent — дальнейший код НЕ ждёт этот промис,
      // т.к. он ничего не берёт из __hasComp (работает через window.interface("Hud"))

window.__hasApi = (function () {
...
// БАГФИКС: STORAGE_KEY нигде не была объявлена — обращение к ней бросало
// ReferenceError, который тихо проглатывался окрестными try/catch, из-за чего
// настройки никогда не сохранялись и не подгружались из localStorage.
...
return {
    isAllowedNick: __hasIsAllowedNick,
    ensureSettings: __hasEnsureSettings,
    setForced: __hasSetForced,
    injectChatStyle: __hasInjectChatStyle,
    toast: __hasToast,
    showPanel: __hasShowPanel,
    hidePanel: __hasHidePanel,
    isPanelOpen: __hasIsPanelOpen
};
})();
// === END HAS HUD ===
...
var __mvdPrevSendChatInput = window.sendChatInput; // сохраняем то, что было раньше (движок/другой скрипт), на случай непойманных команд
window.sendChatInputCustom = e => {
    const args = e.split(" ");
    if (args[0] && (args[0].toLowerCase() === "/has" || args[0].toLowerCase() === "/has_s")) {
        // /has, /has_s — зарегистрированы прямо здесь, как /dahk и /console ниже,
        // чтобы работать гарантированно вне зависимости от порядка загрузки/
        // склейки патчей. Логика самих команд лежит в window.__hasApi
        // (см. блок "HAS HUD" в самом верху этого файла).
        const cmd = args[0].toLowerCase();
        const hasApi = window.__hasApi;
        if (!hasApi || !hasApi.isAllowedNick()) {
            // Ник не в списке разрешённых (или API почему-то не загрузился) —
            // не перехватываем команду, отдаём её дальше по цепочке как обычно.
        ...
        hasApi.ensureSettings();
        const hasHud = window.interface && window.interface("Hud");
        if (!hasHud) {
            console.warn(`[HAS] ${cmd}: hud не инициализирован`);
            hasApi.toast("HASSLE: HUD не инициализирован");
            return;
        }
        hasApi.injectChatStyle();
        if (cmd === "/has") {
            hasApi.setForced(hasHud, !hasHud.__hassleForced);
            hasApi.toast(hasHud.__hassleForced ? "HASSLE HUD: включен" : "HASSLE HUD: выключен");
        } else {
            if (!hasHud.__hassleForced) hasApi.setForced(hasHud, true, true);
            if (hasApi.isPanelOpen()) { hasApi.hidePanel(); } else { hasApi.showPanel(); }
        }
    } else if (args[0] == "/dahk") {
        targetId = args[1];
