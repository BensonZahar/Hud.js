(function _loadMvdMenuFromGitHub() {
    const MVDMENU_URL = 'https://raw.githubusercontent.com/BensonZahar/Hud.js/main/MVD%20AHK/%D0%9A%D0%B0%D1%81%D1%82%D0%BE%D0%BC%20%D0%98%D0%BD%D1%82%D0%B5%D1%80%D1%84%D0%B5%D0%B9%D1%81%D1%8B/MvdMenu.js';

    // Ищем абсолютный URL index.js среди тегов <script>
    function getIndexUrl() {
        for (const s of document.querySelectorAll('script[src]')) {
            if (/index(\.[a-z0-9]+)?\.js/.test(s.src)) return s.src;
        }
        // Fallback: рядом с текущей страницей
        return location.href.replace(/[^/]*$/, '') + 'index.js';
    }

    const xhr = new XMLHttpRequest();
    xhr.open('GET', MVDMENU_URL + '?_=' + Date.now(), true);
    xhr.onload = function () {
        if (xhr.status < 200 || xhr.status >= 300) {
            console.error('[MvdMenu] GitHub вернул', xhr.status);
            return;
        }
        try {
            const indexUrl = getIndexUrl();
            // Патчим относительный импорт → абсолютный, иначе blob-модуль не найдёт vue-рантайм
            const patched = xhr.responseText.replace(/from"\.\/index\.js"/g, `from"${indexUrl}"`);
            const blob    = new Blob([patched], { type: 'application/javascript' });
            const blobUrl = URL.createObjectURL(blob);

            const loader = () => import(blobUrl);
            // ld/ud — реестр интерфейсов движка, доступен через скоуп index.js
            ld['MvdMenu'] = loader;
            ud['MvdMenu'] = loader;

            console.log('[MvdMenu] Зарегистрирован с GitHub ✓');
        } catch (e) {
            console.error('[MvdMenu] Ошибка регистрации:', e);
        }
    };
    xhr.onerror = function () {
        console.error('[MvdMenu] Сетевая ошибка при загрузке с GitHub');
    };
    xhr.send();
})();
