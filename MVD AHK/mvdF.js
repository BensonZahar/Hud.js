...
function __hasCaptureChatState() {
    ...
    } catch (e) { console.warn("[HAS] captureChatState error:", e); }
}
...
function __hasRestoreChatState() {
    ...
    } catch (e) { console.warn("[HAS] restoreChatState error:", e); }
}
...
// ═══════════════════════════════════════════════════════════════
// ГЛАВНЫЙ ПАТЧ: instance-level через Object.defineProperty
// (работает в любом eval-контексте, не требует локальных
//  переменных Mu/o/w/h из модуля Hud.js)
// ═══════════════════════════════════════════════════════════════
function __hasPatchLiveInstance(hud) {
    if (!hud || hud.__hasPatched) return false;
    hud.__hasPatched = true;

    // 1. Устанавливаем реактивное поле
    if (typeof hud.__hassleForced === "undefined") {
        hud.__hassleForced = false;
    }

    // 2. Переопределяем computed isHassleHud через defineProperty
    try {
    ...
    // 3. Подмена Chat на RadmirChat (через $options.components)
    try {
...
// ═══════════════════════════════════════════════════════════════
// КОМАНДЫ /has и /has_s (доступны всем)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// POLLING: ждём инстанс Hud и применяем патч
// (НЕ автовключаем — только готовим патч)
// ═══════════════════════════════════════════════════════════════
