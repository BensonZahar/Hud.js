...
// === HASSLE HUD PATCH (ADAPTED FOR mvdF.js — works via eval) ===
(function(){
console.log("[HAS-mvdF] патч запускается (адаптирован для eval в mvdF.js)");

// ═══════════════════════════════════════════════════════════════════════════════
// АДАПТАЦИЯ ДЛЯ mvdF.js (выполнение через eval)
// ═══════════════════════════════════════════════════════════════════════════════
// При выполнении через eval() в mvdF.js:
// 1. Mu недоступен напрямую (экспортируется из Hud.js)
// 2. Oe/Ao/sr недоступны (объявлены через const в index.js)
// 3. import("./Hud.js") может не сработать из eval
//
// Решение:
// - Получаем компонент через window.interface("Hud").$options
// - Получаем Vue-хелперы через window.App._context или через сам компонент
// - Используем polling, если компонент ещё не загружен
// ═══════════════════════════════════════════════════════════════════════════════
...
// Получаем Vue-хелперы через window.App или через компонент
function __hasFindVueHelpers() {
    var ob = null, cb = null, cc = null;
    
    // Способ 1: Через window.App._context (если доступен)
    try {
        if (window.App && window.App._context) {
            var ctx = window.App._context;
            if (ctx.app && ctx.app._context) {
                var appCtx = ctx.app._context;
                if (typeof appCtx.openBlock === "function") ob = appCtx.openBlock;
                if (typeof appCtx.createBlock === "function") cb = appCtx.createBlock;
                if (typeof appCtx.createCommentVNode === "function") cc = appCtx.createCommentVNode;
            }
        }
    } catch (e) {}
    
    // Способ 2: Через глобальные функции Vue (если экспортируются)
    try {
        if (!ob && typeof window.openBlock === "function") ob = window.openBlock;
        if (!cb && typeof window.createBlock === "function") cb = window.createBlock;
        if (!cc && typeof window.createCommentVNode === "function") cc = window.createCommentVNode;
    } catch (e) {}
    
    // Способ 3: Фолбэк на старые имена (если вдруг доступны)
...
function __hasPatchHudComponent(__hasComp) {
    if (!__hasComp) { console.warn("[HAS-mvdF] __hasPatchHudComponent вызван без компонента"); return; }
    ...
    if (typeof __hasComp.data === "function") {
        ...
        console.log("[HAS-mvdF] data() обёрнут, __hassleForced будет добавлен");
    } else {
        console.warn("[HAS-mvdF] Hud.data не функция — обёртка data() НЕ применена", __hasComp);
    }

    // Патчим computed
    if (__hasComp.computed) {
        __hasComp.computed.isHassleHud = function() { return this.__hassleForced; };
        console.log("[HAS-mvdF] computed.isHassleHud переопределён");
    } else {
        console.warn("[HAS-mvdF] Hud.computed отсутствует — computed НЕ переопределён", __hasComp);
    }

    // Патчим компоненты (Chat)
    if (__hasComp.components && __hasComp.components.RadmirChat) {
        ...
        console.log("[HAS-mvdF] components.Chat подменён на RadmirChat, дефолты форсированы");
    } else {
        console.warn("[HAS-mvdF] Hud.components.RadmirChat не найден — подмена чат-компонента НЕ выполнена", __hasComp);
    }

    // Патчим VoiceChat (если есть хелперы)
    if (__hasComp.components && __hasComp.components.HudHassle && __hasComp.components.HudRadmir) {
        ...
        if (!__hasBlockHelpersOk) {
            console.warn("[HAS-mvdF] openBlock/createBlock/createCommentVNode недоступны — VoiceChat-патч отключён");
        }
        ...
        if (__hasBlockHelpersOk && __hasVoiceChatComp && typeof __hasHudHassleComp.render === "function") {
            ...
            __hasHudHassleComp.render = function() {
                ...
                } catch (err) { console.warn("[HAS-mvdF] не удалось добавить VoiceChat внутрь HudHassle", err); }
                return vnode;
            };
            console.log("[HAS-mvdF] HudHassle.render обёрнут — VoiceChat дорисовывается вручную");
        } else {
            console.warn("[HAS-mvdF] не нашёл HudRadmir.components.VoiceChat, HudHassle.render или Vue-хелперы — VoiceChat в Hassle НЕ добавлен", __hasComp.components);
        }
    }

    // Патчим render (фикс Fragment key)
    ...
    if (typeof __hasComp.render === "function") {
        ...
        __hasComp.render = function() {
            ...
            try {
                ...
                console.warn("[HAS-mvdF] fixChatFragmentKey error:", err);
            }
            return vnode;
        };
        console.log("[HAS-mvdF] Hud.render обёрнут — Fragment key чата будет зафиксирован");
    }
}

// ─── Точка входа: получаем компонент через polling ───
(function __hasInitCompPatch() {
    var comp = null;
    
    // Способ 1: Через window.interface("Hud").$options
    try {
        var hudInstance = window.interface && window.interface("Hud");
        if (hudInstance && hudInstance.$options && __hasLooksLikeHudComp(hudInstance.$options)) {
            comp = hudInstance.$options;
        }
    } catch (e) {}
    
    // Способ 2: Через import("./Hud.js") с retry
    if (!comp) {
        console.log("[HAS-mvdF] Компонент Hud не найден через interface — подгружаю через import()...");
        ...
        var tryLoad = function() {
            ...
            // Сначала пробуем через interface
            try {
                var hudInstance = window.interface && window.interface("Hud");
                if (hudInstance && hudInstance.$options && __hasLooksLikeHudComp(hudInstance.$options)) {
                    comp = hudInstance.$options;
                    __hasPatchHudComponent(comp);
                    console.log("[HAS-mvdF] ✅ Компонент Hud получен через interface");
                    return;
                }
            } catch (e) {}
            
            // Если не получилось — пробуем import
            if (retryCount <= 3) {
                import("./Hud.js").then(function(mod) {
                    ...
                    if (__hasLooksLikeHudComp(loadedComp)) {
                        ...
                        console.log("[HAS-mvdF] ✅ Компонент Hud получен через import()");
                    } else {
                        console.warn("[HAS-mvdF] Компонент не похож на Hud — retry", retryCount);
                        if (retryCount < maxRetries) setTimeout(tryLoad, 500);
                    }
                }).catch(function(err) {
                    console.warn("[HAS-mvdF] import() не сработал — retry", retryCount, err.message);
                    if (retryCount < maxRetries) setTimeout(tryLoad, 500);
                });
            } else if (retryCount < maxRetries) {
                setTimeout(tryLoad, 500);
            } else {
                console.warn("[HAS-mvdF] ❌ Не удалось получить компонент Hud после", maxRetries, "попыток");
            }
        };
...
function __hasCaptureChatState() {
    try {
        ...
        if (hud && hud.$refs && hud.$refs.chat) {
            ...
            __mvdChatInst = chat;
            console.log("[HAS-mvdF] chat state сохранён, messages:", __mvdChatMessages.length);
        }
    } catch (e) {
        console.warn("[HAS-mvdF] captureChatState error:", e);
    }
}
    ...
    try {
        ...
        if (hud && hud.$refs && hud.$refs.chat) {
            ...
            if (chat !== __mvdChatInst) {
                ...
                __mvdChatInst = chat;
                console.log("[HAS-mvdF] ✅ chat state восстановлен на новом инстансе");
            }
        }
    } catch (e) {
        console.warn("[HAS-mvdF] restoreChatState error:", e);
    }
}
...
function __hasGetOwnNick() {
    ...
    catch (e) { console.warn("[HAS-mvdF] ошибка получения ника", e); return null; }
}
...
function __hasEnsureSettings() {
    ...
    if (nick && nick !== __hasSettingsNick) {
        ...
        console.log("[HAS-mvdF] настройки загружены для ника", nick, settings);
    }
    return settings;
}
function __hasSaveSettings() {
    ...
    } catch (e) { console.warn("[HAS-mvdF] ошибка сохранения настроек", e); }
}
...
function __hasInjectChatStyle() {
    if (document.getElementById("__has-chat-style")) { console.log("[HAS-mvdF] стиль уже вставлен"); return; }
    ...
    console.log("[HAS-mvdF] стиль вставлен в head");
}
...
function __hasApplyCSSVars() {
    ...
    console.log("[HAS-mvdF] CSS-переменные применены");
}

function __hasApplyToHud() {
    var hud = window.interface && window.interface("Hud");
    if (!hud) { console.warn("[HAS-mvdF] __hasApplyToHud: hud не найден"); return; }
    ...
    if (hud.voiceChat) {
        ...
        console.log("[HAS-mvdF] voiceChat.show/showButtons принудительно включены");
    } else {
        console.warn("[HAS-mvdF] hud.voiceChat не найден на инстансе");
    }
}
...
function __hasSetForced(hud, val, silent) {
    ...
    console.log("[HAS-mvdF] __hassleForced = ", val, "| body class: ", document.body.classList.contains("__has-chat-hassle-pos"));
    ...
    setTimeout(function() {
        console.log("[HAS-mvdF] DOM-проверка: ", {
...
window.sendChatInput = function(e) {
    ...
    if (cmd === "/has") {
        var hud = window.interface && window.interface("Hud");
        if (!hud) {
            console.warn("[HAS-mvdF] /has: hud не инициализирован");
        ...
        var hud = window.interface && window.interface("Hud");
        if (!hud) {
            console.warn("[HAS-mvdF] /has_s: hud не инициализирован");
            __hasToast("HASSLE: HUD \u043d\u0435 \u0438\u043d\u0438\u0446\u0438\u0430\u043b\u0438\u0437\u0438\u0440\u043e\u0432\u0430\u043d");
            return;
        }
        __hasInjectChatStyle();
...
window.onUpdatePlayersList = function(e) {
    ...
    } catch (err) { console.warn("[HAS-mvdF] onUpdatePlayersList ошибка", err); }
...
(function __hasAutoInit() {
    ...
    var timer = setInterval(function() {
        ...
        if (hud && __hasIsAllowedNick()) {
            clearInterval(timer);
            console.log("[HAS-mvdF] автозапуск: HUD найден, ник разрешён");
            __hasEnsureSettings();
            __hasInjectChatStyle();
            ...
            console.warn("[HAS-mvdF] автозапуск: не дождались HUD/разрешённого ника");
            clearInterval(timer);
        }
    }, 150);
})();
...
setInterval(function() {
    ...
    if (hud && hud.$refs && hud.$refs.chat && __mvdChatAddFn) {
        if (hud.$refs.chat !== __mvdChatInst) {
            console.log("[HAS-mvdF] ⚠️ Обнаружен новый инстанс чата — восстанавливаем state");
...
// === END HASSLE HUD PATCH (mvdF.js version) ===
