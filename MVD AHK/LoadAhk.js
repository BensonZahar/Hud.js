...
// ── Загрузчик MvdMenu_dom.js (vanilla DOM, без Vue, без IntLoad) ────────────
function loadMvdMenuDom(retries = 3) {
    const mvdDomUrl =
        `https://raw.githubusercontent.com/${username}/${repo}/main/` +
        `${encodeURIComponent(folder)}/MvdMenu_dom.js` +
        `?_=${Date.now()}`;
    const xhr = new XMLHttpRequest();
    xhr.open('GET', mvdDomUrl, true);
    xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
            eval(xhr.responseText);
            console.log('[LoadAhk] MvdMenu_dom.js загружен — window._mvdShowMenu готов');
        } else {
            console.error(`[LoadAhk] MvdMenu_dom.js: HTTP ${xhr.status}`);
            if (retries > 0) setTimeout(() => loadMvdMenuDom(retries - 1), 2000);
        }
    };
    xhr.onerror = function () {
        console.error('[LoadAhk] MvdMenu_dom.js: ошибка сети');
        if (retries > 0) setTimeout(() => loadMvdMenuDom(retries - 1), 2000);
    };
    xhr.send();
}
...
function loadScriptFromGitHub(username, repo, folder, filename, retries = 5) {
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            // ── Перехват window.showPovsednevMenuPage (меню → экран povsednev) ──
            // Определяется в mvdF.js: ставит _mvdMenuTargetId/_mvdMenuStartScreen
            // и вызывает openInterface('MvdMenu').
            // Мы заменяем на _mvdShowMenu, которая живёт в MvdMenu.dom.js.
            var _origShowPovsed = window.showPovsednevMenuPage;
            window.showPovsednevMenuPage = function(targetId) {
                window._mvdMenuTargetId = (targetId !== undefined) ? targetId : null;
                window._mvdMenuStartScreen = 'povsednev';
                if (typeof window._mvdShowMenu === 'function') {
                    window._mvdShowMenu(targetId, 'povsednev');
                } else if (typeof _origShowPovsed === 'function') {
                    _origShowPovsed(targetId); // fallback: MvdMenu.dom.js ещё не загрузился
                }
            };
            window._origShowPovsednevMenuPage = _origShowPovsed;
            // ── Перехват window.showMvdMainMenuPage (меню → экран main) ─────────
            // Вызывается при нажатии MENU_KEY (хоткей из LoadAhk/установщика).
            var _origShowMain = window.showMvdMainMenuPage;
            window.showMvdMainMenuPage = function(targetId) {
                window._mvdMenuTargetId = (targetId !== undefined) ? targetId : null;
                window._mvdMenuStartScreen = 'main';
                if (typeof window._mvdShowMenu === 'function') {
                    window._mvdShowMenu(targetId, 'main');
                } else if (typeof _origShowMain === 'function') {
                    _origShowMain(targetId);
                }
            };
            window._origShowMvdMainMenuPage = _origShowMain;
            // ── END перехваты ─────────────────────────────────────────────────
            // Явно устанавливаем window.AUTO_GRAB после eval
            if (AUTO_GRAB) window.AUTO_GRAB = true;
            // ── Грузим MvdMenu.dom.js с GitHub (async, не блокирует) ──────────
            loadMvdMenuDom();
