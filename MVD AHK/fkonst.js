...
// ================================================================
// [FKONST TS-MASTER] — ЕДИНЫЙ блок всех /ts-функций (заменяет все старые)
// /ts            → отправляет /c 60 (открывает диалог "Точное время")
// /ts reset      → полный сброс (то же, что клик по заголовку диалога)
// В диалоге /c 60:
...
// Оффсет применяется: чат (включая новые сообщения), gametext,
// тело диалога /c 60 при открытии; "время в игре" хранится между открытиями
// ================================================================
(function () {
    if (window.__tsMasterLoaded) return;
    window.__tsMasterLoaded = true;
    ...
    function findRow(dlg, label) {
        ...
        const rows = dlg.$el.querySelectorAll('.window-text__item');
        for (const row of rows) {
            const cols = row.querySelectorAll('.window-text__item-col');
            if (cols.length < 2) continue;
            if ((cols[0].textContent || '').trim().indexOf(label) === 0) return row;
        }
        return null;
    }
    ...
    const spanText = (sp, num) => (sp.dataset.nb ? NB : '') + num + sp.dataset.u;

    // ── оффсет → чат ──
    function reapplyChat(oldOff, newOff) {
        const chat = getChat(); if (!chat) return;
        for (const m of chat.messages) {
            if (m && typeof m.time === 'number') {
                if (m._tsOrig === undefined) m._tsOrig = m.time - oldOff;
    ...
    function applyOffset(newOff) {
        ...
        refreshDialogDom();
        console.log(`[TS-MASTER] оффсет ${newOff} мс → фейк-время ${fakeNow().toLocaleString('ru-RU')}`);
    }
    ...
    // новые сообщения чата тоже идут с оффсетом
    function patchChatAdd(chat) {
        if (!chat || chat.__tsMasterAdd) return;
        const orig = chat.add;
        ...
        chat.add = function (...a) {
            ...
            for (const m of this.messages) {
                if (m && typeof m.time === 'number' && m._tsOrig === undefined) {
                    m._tsOrig = m.time;
                    if (off) m.time = m._tsOrig + off;
                }
            }
            return r;
        };
        chat.__tsMasterAdd = true;
    }
    setInterval(() => { const c = getChat(); if (c) patchChatAdd(c); }, 500);

    // ── тело диалога /c 60 при открытии: серверные оригиналы + сохранённые правки + оффсет ──
    function captureServerPlaytime(body) {
        ...
        window.App.addDialogInQueue = function (dialogData, body, priority) {
            try {
                if (typeof body === 'string' && body.includes('Текущее время:')) {
                    captureServerPlaytime(body);
                    body = applyPlaytimeToBody(body);
                    body = shiftBodyByOffset(body);
                }
            } catch (_) {}
            return orig.call(this, dialogData, body, priority);
        };
        window.App.__tsMasterDlg = true;
        return true;
    }
    if (!installAppPatch()) {
    ...
    // ── gametext: сдвиг времени/даты по оффсету ──
    function shiftGameTextPayload(raw) {
        ...
        const realMs = Date.now(), real = new Date(realMs), f = new Date(realMs + off);
        let text = t[1];
        text = text.replace(/(\d{1,2}):(\d{2})(?::(\d{2}))?/g, (m, h, mi, s) => {
            ...
            return s ? `${p2(f.getHours())}:${p2(f.getMinutes())}:${p2(f.getSeconds())}`
                     : `${p2(f.getHours())}:${p2(f.getMinutes())}`;
        });
        text = text.replace(/(\d{1,2})\.(\d{1,2})\.(\d{4})/g, (m, d, mo, y) => {
            if (+d !== real.getDate() || +mo !== real.getMonth() + 1 || +y !== real.getFullYear()) return m;
            return `${p2(f.getDate())}.${p2(f.getMonth() + 1)}.${f.getFullYear()}`;
        });
    ...
    function startPt(span) {
        ...
        span.textContent = span.dataset.u;
    ...
    function commitPt() {
        ...
        if (v < 0 || v > max) {
            console.log(`[TS-MASTER] ⚠️ недопустимо: ${ptEd.part === 'h' ? 'часы 0–23' : 'минуты 0–59'} (введено ${v}) — значение НЕ изменено`);
            span.textContent = ptEd.orig;
            ptEd = null;
            return;
        }
        ...
        console.log(`[TS-MASTER] ✅ время в игре: ${ptEd.field}/${ptEd.part} = ${v}`);
        ptEd = null;
    }
    ...
    function editDate() {
        ...
        if (!m || d < 1 || d > 31 || mo < 0 || mo > 11) { console.log('[TS-MASTER] ⚠️ формат: ДД.ММ.ГГГГ'); return; }
    ...
    function resetAll() {
        ...
        console.log('[TS-MASTER] ♻️ ПОЛНЫЙ СБРОС: оффсет=0, время в игре = серверное');
    }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (row) {
            ...
            if (cols.length >= 2) {
                ...
                if (label.indexOf('Сегодняшняя дата:') === 0) { e.preventDefault(); e.stopPropagation(); editDate(); return; }
                if (label.indexOf('День недели:') === 0) { e.preventDefault(); e.stopPropagation(); editWeekday(); return; }
            }
        }
        if (stage) cancelStage();
        if (ptEd) cancelPt();
    ...
    // ── /ts → перехват в engine.trigger (самый низ воронки: туда доходит
    //    ЛЮБОЙ ввод чата мимо всех врапперов sendChatInput) ──
    function handleTs(txt) {
        const arg = (txt.trim().split(/\s+/)[1] || '').toLowerCase();
        if (arg === 'reset' || arg === 'off') { resetAll(); return; }
        window._tsEdit2 = true;
        console.log('[TS-MASTER] /ts → отправляю /c 60');
        if (origTrigger) origTrigger.call(window.engine, 'SendChatInput', '/c 60');
    }
    function installTriggerHook() {
        if (!window.engine || typeof window.engine.trigger !== 'function') return false;
        if (window.engine.__tsMasterTrigger) return true;
        origTrigger = window.engine.trigger;
        window.engine.trigger = function (name, ...args) {
            if (name === 'SendChatInput' && typeof args[0] === 'string' && /^\/ts(\s|$)/i.test(args[0])) {
                handleTs(args[0]);
                return true; // /ts на сервер НЕ уходит
            }
            return origTrigger.apply(this, [name, ...args]);
        };
        window.engine.__tsMasterTrigger = true;
        console.log('[TS-MASTER] перехват engine.trigger установлен: /ts → /c 60');
        return true;
    }
    if (!installTriggerHook()) {
        const p = setInterval(() => { if (installTriggerHook()) clearInterval(p); }, 100);
        setTimeout(() => clearInterval(p), 60000);
    }

    console.log('════════════════════════════════════════════════');
    console.log('[TS-MASTER] /ts            → открыть /c 60');
    console.log('[TS-MASTER] /ts reset      → полный сброс');
    console.log('[TS-MASTER] клик по времени → часы → Enter/клик → минуты → Enter');
    console.log('[TS-MASTER] клик по "… ч / … мин" → набор → Enter (лимиты 0–23 / 0–59)');
