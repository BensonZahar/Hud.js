...
if (!window.intBrowserKeyHandlerInstalled) {
    // Сохраняем ТЕКУЩИЙ обработчик (который уже может быть установлен)
    const previousOnKeyDown = window.onkeydown;
    window.onkeydown = function(e) {
        const keyCode = e.keyCode || e.which;
       
        // Только если режим просмотра включен
        if (window.intBrowserMode && !window.inputFocus) {
            // Стрелка ВПРАВО (39) - следующий интерфейс
            if (keyCode === 39) {
                e.preventDefault();
            ...
            // Стрелка ВЛЕВО (37) - предыдущий интерфейс
            if (keyCode === 37) {
                e.preventDefault();
            ...
            // ESC (27) - выход из режима просмотра
            if (keyCode === 27) {
                e.preventDefault();
        ...
        // Вызываем предыдущий обработчик для всех остальных клавиш
...
// Функция безопасного показа уведомления
if (!window.safeNotification) {
...
// ============================================
// Сохраняем текущий обработчик команд (если он есть)
...
window.sendChatInputCustom = function(e) {
    ...
    // ===================== Все остальные команды передаем предыдущему обработчику =====================
    if (typeof previousSendChatInput === 'function') {
        return previousSendChatInput(e);
    } else {
        // Если нет предыдущего обработчика - используем стандартную отправку
        window.App.developmentMode || engine.trigger("SendChatInput", e);
    }
};
