...
// ⚠️ ИСПОЛЬЗУЕТ ЦЕПОЧКУ ОБРАБОТЧИКОВ
// ============================================

// Сохраняем ссылку на ТЕКУЩИЙ обработчик команд
if (!window.interfaceLoaderCommandsInstalled) {
    // Сохраняем то, что было ДО нас
    window.previousChatInputHandler = window.sendChatInput || window.sendChatInputCustom;

    // Наша функция обработки команд интерфейсов
    window.handleInterfaceCommands = function(e) {
        const args = e.trim().split(" ");
        ...
        return false; // Команда НЕ обработана, передаём дальше
    };

    // Создаём НОВЫЙ обработчик, который проверяет сначала наши команды
    window.sendChatInputCustom = function(e) {
        // Сначала проверяем наши команды интерфейсов
        if (window.handleInterfaceCommands(e)) {
            return; // Если наша команда - не передаём дальше
        }
       
        // Если это НЕ наша команда - передаём предыдущему обработчику
        if (window.previousChatInputHandler) {
            return window.previousChatInputHandler(e);
        } else {
            // Если нет предыдущего обработчика - стандартная отправка
            window.App.developmentMode || engine.trigger("SendChatInput", e);
        }
    };
    ...
    window.interfaceLoaderCommandsInstalled = true;
    console.log('✅ Команды управления интерфейсами установлены');
}
