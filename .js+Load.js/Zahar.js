...
const config = {
  ...
  afkCycle: {
    ...
    restartBehavior: 'q' // Новый флаг: 'q' или 'rec' для поведения при рестарте сервера
  },
...
function showAFKReconnectMenu(chatId, messageId, uniqueIdParam, mode) {
  if (!config.autoReconnectEnabled) {
    // Если реконнект отключен глобально, пропускаем и активируем AFK без реконнекта
    activateAFKNight(mode, false);
    return;
  }
  const replyMarkup = {
    inline_keyboard: [
      [
        createButton("Реконнект 🟢", `afk_n_reconnect_on_${uniqueIdParam}_${mode}`),
        createButton("Реконнект 🔴", `afk_n_reconnect_off_${uniqueIdParam}_${mode}`)
      ],
...
function showAFKRestartBehaviorMenu(chatId, messageId, uniqueIdParam, mode) {
  const replyMarkup = {
    inline_keyboard: [
      [
        createButton("/q", `afk_n_restart_q_${uniqueIdParam}_${mode}`),
        createButton("/rec 5 через 4 мин", `afk_n_restart_rec_${uniqueIdParam}_${mode}`)
      ],
      [createButton("⬅️ Вернуться назад", `afk_n_reconnect_on_${uniqueIdParam}_${mode}`)]
    ]
  };
  editMessageReplyMarkup(chatId, messageId, replyMarkup);
}
...
function showMovementControlsMenu(chatId, messageId, isNotification = false) {
  ...
  const backButton = isNotification ? [[createButton("⬅️ Вернуться назад", `back_to_notification_${uniqueId}`)]] : [[createButton("⬅️ Вернуться назад", `show_local_functions_${uniqueId}`)]];
...
function processUpdates(updates) {
  for (const update of updates) {
    ...
    if (update.message) {
      ...
      const isGlobalCommand = message.startsWith('global_') || message.startsWith('afk_n_') || message.startsWith('show_payday_options_') || message.startsWith('show_soob_options_') || message.startsWith('show_mesto_options_') || message.startsWith('show_radio_options_') || message.startsWith('show_warning_options_') || message.startsWith('show_global_functions_');
      ...
      if (message.startsWith('show_controls_')) {
        ...
        const parts = message.replace('afk_n_reconnect_on_', '').split('_');
        callbackUniqueId = parts[0];
        callbackMode = parts[1];
        showAFKRestartBehaviorMenu(chatId, messageId, callbackUniqueId, callbackMode);
        answerCallbackQuery(callbackQueryId);
        return;
      } else if (message.startsWith('afk_n_reconnect_off_')) {
        const parts = message.replace('afk_n_reconnect_off_', '').split('_');
        callbackUniqueId = parts[0];
        callbackMode = parts[1];
        activateAFKNight(callbackMode, false);
      } else if (message.startsWith('afk_n_restart_q_')) {
        const parts = message.replace('afk_n_restart_q_', '').split('_');
        callbackUniqueId = parts[0];
        callbackMode = parts[1];
        activateAFKNight(callbackMode, true, 'q');
      } else if (message.startsWith('afk_n_restart_rec_')) {
        const parts = message.replace('afk_n_restart_rec_', '').split('_');
        callbackUniqueId = parts[0];
        callbackMode = parts[1];
        activateAFKNight(callbackMode, true, 'rec');
      ...
      const isForThisBot = isGlobalCommand || (callbackUniqueId && callbackUniqueId === uniqueId) || (update.callback_query.message.text && update.callback_query.message.text.includes(displayName)) || (update.callback_query.message.reply_to_message && update.callback_query.message.reply_to_message.text && update.callback_query.message.reply_to_message.text.includes(displayName));
      if (!isForThisBot) {
...
function activateAFKNight(mode, reconnectEnabled = false, restartBehavior = 'q') {
  ...
  config.afkCycle.reconnectEnabled = reconnectEnabled;
  config.afkCycle.restartBehavior = restartBehavior;
  const reconnectText = reconnectEnabled ? 'с реконнектом' : 'без реконнекта';
  const behaviorText = reconnectEnabled ? (restartBehavior === 'q' ? '/q' : '/rec 5 через 4 мин') : '';
  sendToTelegram(`🔄 <b>AFK режим (${mode}) активирован для ${displayName} ${reconnectText} ${behaviorText}</b>\nID из HUD: ${hudId}\nФорматы: ${idFormats.join(', ')}`, false, null);
  startAFKCycle();
}
...
function enterPauseUntilEnd() {
  if (config.afkCycle.reconnectEnabled) {
    ...
    sendChatInput("/rec 5");
    debugLog(`Реконнект: /rec 5 отправлено, автовход отключен до конца цикла для ${displayName}`);
    sendToTelegram(`🔄 <b>Реконнект после 25 мин: /rec 5 отправлено, автовход отключен (${displayName})</b>`, false, null);
  } else {
    try {
      if (typeof openInterface === 'function') {
        openInterface("PauseMenu");
...
function handlePayDayTimeMessage() {
  ...
  config.afkCycle.mainTimer = setTimeout(() => {
    if (config.afkCycle.reconnectEnabled) {
      ...
      initializeAutoLogin();
      sendChatInput("/rec 5");
      debugLog(`Реконнект: автовход включен, /rec 5 отправлено перед следующим PayDay для ${displayName}`);
      sendToTelegram(`🔄 <b>Реконнект перед следующим PayDay: /rec 5 отправлено (${displayName})</b>`, false, null);
    } else {
      try {
        if (typeof closeInterface === 'function') {
          closeInterface("PauseMenu");
...
function initializeChatMonitor() {
  ...
  window.OnChatAddMessage = function(e, i, t) {
    ...
    // Для отладки, выводим сообщения в чат // console.log(msg); // сооб в чат
    ...
    if (config.afkSettings.active && config.afkCycle.active && msg.includes("Сервер возобновит работу в течение минуты...")) {
      ...
      if (config.afkCycle.restartBehavior === 'q') {
        sendChatInput("/q");
        sendToTelegram(`⚡ <b>Автоматически отправлено /q (${displayName})</b>\nПо условию AFK ночь: Сервер возобновит работу`, false, null);
      } else if (config.afkCycle.restartBehavior === 'rec') {
        setTimeout(() => {
          sendChatInput("/rec 5");
          sendToTelegram(`🔄 <b>Автоматически отправлено /rec 5 через 4 мин (${displayName})</b>\nПо условию AFK ночь: Сервер возобновит работу`, false, null);
        }, 4 * 60 * 1000); // 4 минуты
      }
    }
    ...
    if ((lowerCaseMessage.indexOf("администратор") !== -1 && lowerCaseMessage.indexOf("для") !== -1) || (msg.includes("[A]") && msg.includes("((")) || (lowerCaseMessage.includes("подбросил") && (currentTime - config.lastPodbrosTime > config.podbrosCooldown || config.podbrosCounter < 2))) {
    ...
    if (!isNonRPMessage(msg) && getRankKeywords().some(kw => lowerCaseMessage.includes(kw)) && (lowerCaseMessage.indexOf("строй") !== -1 || lowerCaseMessage.indexOf("сбор") !== -1 || lowerCaseMessage.indexOf("готовность") !== -1 || lowerCaseMessage.indexOf("конф") !== -1) && (chatRadius === CHAT_RADIUS.RADIO)) {
