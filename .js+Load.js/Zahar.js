...
autoReconnectEnabled: true // Добавлена опция автореконнекта
};
const config = {
...userConfig,
lastUpdateId: 0,
...
createButton("С реконнектом", `afk_n_reconnect_${uniqueIdParam}`),
createButton("Без реконнекта", `afk_n_no_reconnect_${uniqueIdParam}`)
]);
} else {
...
replyMarkup.inline_keyboard.push([createButton("⬅️ Вернуться назад", `show_global_functions_${uniqueIdParam}`)]);
editMessageReplyMarkup(chatId, messageId, replyMarkup);
}
...
function showAFKPausesMenu(chatId, messageId, uniqueIdParam) {
const replyMarkup = {
inline_keyboard: [
[
createButton("С паузами", `afk_n_with_pauses_${uniqueIdParam}`),
...
} else if (message.startsWith('afk_n_reconnect_')) {
callbackUniqueId = message.replace('afk_n_reconnect_', '');
} else if (message.startsWith('afk_n_no_reconnect_')) {
callbackUniqueId = message.replace('afk_n_no_reconnect_', '');
}
...
sendToTelegram(`✅ <b>Автореконнект включен для ${displayName}</b>`, false, null, config.notificationDeleteDelay);
showAFKPausesMenu(chatId, messageId, callbackUniqueId);
} else if (message.startsWith(`afk_n_no_reconnect_`)) {
autoLoginConfig.enabled = false;
sendToTelegram(`❌ <b>Автореконнект отключен для ${displayName}</b>`, false, null, config.notificationDeleteDelay);
showAFKPausesMenu(chatId, messageId, callbackUniqueId);
} else if (message.startsWith(`global_afk_`)) {
