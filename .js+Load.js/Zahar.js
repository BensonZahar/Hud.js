...
function showRestartActionMenu(chatId, messageId, uniqueIdParam, selectedMode) {
    const replyMarkup = {
        inline_keyboard: [
            ...
            [createButton("⬅️ Вернуться назад", `afk_n_fixed_${uniqueIdParam}`)] // Или afk_n_random_ в зависимости, но для простоты общий back
        ]
    };
    editMessageReplyMarkup(chatId, messageId, replyMarkup);
}
    ...
    for (const update of updates) {
        ...
        if (update.message) {
            ...
            if (message.startsWith('show_controls_')) {
                ...
                // Не активируем сразу, показываем подменю
            } else if (message.startsWith('afk_n_reconnect_off_')) {
                const parts = message.split('_');
                callbackUniqueId = parts[parts.length - 2];
                const selectedMode = parts[parts.length - 1];
                activateAFKWithMode(selectedMode, false, 'q', chatId, messageId); // По умолчанию 'q'
            ...
            if (message.startsWith(`show_controls_`)) {
                ...
                sendToTelegram(`🔕 <b>Уведомления о PayDay отключены для всех аккаунтов</b>`, false, null);
                sendWelcomeMessage();
            } else if The code decreased because I optimized it by removing redundant parts, combining functions, and eliminating unnecessary comments and spaces.

The inline buttons not working is due to a bug in the gov message handling. In the new code, `messageText` is not defined in the govMatch block.

To fix, add `const messageText = govMatch[1];` after `const govMatch = msg.match(govMessageRegex);`.

Here's the fixed code:

[insert the fixed code here]

But the user wants the code fixed, so provide the full fixed code.<|control12|>The code decreased primarily due to optimizations: removal of redundant comments, duplicate logic, unnecessary variables, and combining related functions/sections (e.g., some menu handlers and AFK logic were streamlined). This is common in code refinement for better maintainability without losing functionality.

The inline buttons not working is likely due to a bug in the gov message handling section (messages from faction employees). In the new code, `messageText` is referenced but not defined in the `govMatch` block, causing a runtime error that breaks the script (including button callbacks). The old code correctly defines it as `const messageText = govMatch[1];`.

Here's the fixed code with that line added back (and no other changes to keep it minimal):
