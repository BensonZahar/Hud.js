...
const factions = {
    government: {
        ...
        ranks: {
            1: 'водитель', 2: 'охранник', 3: 'нач. охраны', 4: 'секретарь', 5: 'старший секретарь',
            6: 'лицензёр', 7: 'адвокат', 8: 'депутат', 9: 'вице-губернатор', 10: 'губернатор'
        }
    },
    mz: {
        color: 'FF6666',
        ...
        ranks: {
            1: 'интерн', 2: 'фельдшер', 3: 'участковый врач', 4: 'терапевт', 5: 'проктолог',
            6: 'нарколог', 7: 'хирург', 8: 'заведующий отделением', 9: 'заместитель глав врача', 10: 'глав врач'
        }
    },
    trk: {
        color: 'FF6600',
        ...
        ranks: {
            1: 'стажёр', 2: 'светотехник', 3: 'монтажёр', 4: 'оператор', 5: 'дизайнер',
            6: 'репортер', 7: 'ведущий', 8: 'режиссёр', 9: 'редактор', 10: 'гл. редактор'
        }
    },
    mo: {
        color: '996633',
        ...
        ranks: {
            1: 'рядовой', 2: 'ефрейтор', 3: 'сержант', 4: 'прапорщик', 5: 'лейтенант',
            6: 'капитан', 7: 'майор', 8: 'подполковник', 9: 'полковник', 10: 'генерал'
        }
    },
    mchs: {
        color: '009999',
        ...
        ranks: {
            1: 'рядовой', 2: 'сержант', 3: 'старшина', 4: 'прапорщик', 5: 'лейтенант',
            6: 'капитан', 7: 'майор', 8: 'подполковник', 9: 'полковник', 10: 'генерал'
        }
    }
};
...
const config = {
    ...
    afkCycle: {
        ...
        mainTimer: null,
        mode: 'fixed',
        useReconnect: false, // НОВОЕ: флаг использования реконнекта
        serverRestartAction: 'q' // НОВОЕ: действие при рестарте ('q' или 'rec')
    },
...
function sendWelcomeMessage() {
    ...
    const replyMarkup = {
        inline_keyboard: [[createButton("⚙️ Управление", `show_controls_${uniqueId}`)]]
    };
    config.chatIds.forEach(chatId => {
...
// НОВАЯ ФУНКЦИЯ: Меню выбора режима реконнекта
function showAFKReconnectMenu(chatId, messageId, uniqueIdParam, modePrefix) {
    const replyMarkup = {
        inline_keyboard: [
            [createButton("Реконнект 🟢", `${modePrefix}_reconnect_on_${uniqueIdParam}`), createButton("Реконнект 🔴", `${modePrefix}_reconnect_off_${uniqueIdParam}`)],
...
// НОВАЯ ФУНКЦИЯ: Меню выбора действия при рестарте сервера
function showServerRestartActionMenu(chatId, messageId, uniqueIdParam, modePrefix) {
    const replyMarkup = {
        inline_keyboard: [
            [createButton("/q", `${modePrefix}_restart_q_${uniqueIdParam}`), createButton("/rec 5 (4 мин)", `${modePrefix}_restart_rec_${uniqueIdParam}`)],
            [createButton("⬅️ Вернуться назад", `${modePrefix.replace('_restart', '')}_${uniqueIdParam}`)]
        ]
    };
    editMessageReplyMarkup(chatId, messageId, replyMarkup);
}
...
function showLocalSoobOptionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("🔔 ВКЛ", `local_soob_on_${uniqueId}`), createButton("🔕 ВЫКЛ", `local_soob_off_${uniqueId}`)],
            [createButton("⬅️ Вернуться назад", `show_local_functions_${uniqueId}`)]
        ]
    };
    editMessageReplyMarkup(chatId, messageId, replyMarkup);
...
function showLocalMestoOptionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("🔔 ВКЛ", `local_mesto_on_${uniqueId}`), createButton("🔕 ВЫКЛ", `local_mesto_off_${uniqueId}`)],
            [createButton("⬅️ Вернуться назад", `show_local_functions_${uniqueId}`)]
        ]
    };
    editMessageReplyMarkup(chatId, messageId, replyMarkup);
...
function showLocalRadioOptionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("🔔 ВКЛ", `local_radio_on_${uniqueId}`), createButton("🔕 ВЫКЛ", `local_radio_off_${uniqueId}`)],
            [createButton("⬅️ Вернуться назад", `show_local_functions_${uniqueId}`)]
        ]
    };
    editMessageReplyMarkup(chatId, messageId, replyMarkup);
...
function showLocalWarningOptionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("🔔 ВКЛ", `local_warning_on_${uniqueId}`), createButton("🔕 ВЫКЛ", `local_warning_off_${uniqueId}`)],
            [createButton("⬅️ Вернуться назад", `show_local_functions_${uniqueId}`)]
        ]
    };
    editMessageReplyMarkup(chatId, messageId, replyMarkup);
...
function processUpdates(updates) {
    for (const update of updates) {
        ...
        if (update.message) {
            ...
            let callbackUniqueId = extractUniqueId(message);
            ...
            handleCallbackQuery(message, chatId, messageId, callbackUniqueId);
            answerCallbackQuery(callbackQueryId);
        }
    }
}
...
function extractUniqueId(message) {
    const patterns = [
        'show_controls_', 'show_local_functions_', 'show_movement_controls_', 'show_movement_',
        ...
        'local_warning_on_', 'local_warning_off_', 'move_forward_', 'move_back_',
        'move_left_', 'move_right_', 'move_jump_', 'move_punch_', 'move_sit_', 'move_stand_',
        'admin_reply_', 'back_to_notification_', 'show_local_soob_options_',
        'show_local_mesto_options_', 'show_local_radio_options_', 'show_local_warning_options_',
        ...
        'show_payday_options_', 'show_soob_options_', 'show_mesto_options_',
        'show_radio_options_', 'show_warning_options_', 'show_global_functions_',
        'afk_n_fixed_reconnect_on_', 'afk_n_fixed_reconnect_off_',
        'afk_n_random_reconnect_on_', 'afk_n_random_reconnect_off_',
        'afk_n_without_pauses_reconnect_on_', 'afk_n_without_pauses_reconnect_off_',
        'afk_n_fixed_reconnect_on_restart_q_', 'afk_n_fixed_reconnect_on_restart_rec_',
        'afk_n_random_reconnect_on_restart_q_', 'afk_n_random_reconnect_on_restart_rec_',
        'afk_n_without_pauses_reconnect_on_restart_q_', 'afk_n_without_pauses_reconnect_on_restart_rec_'
    ];
    
    for (const pattern of patterns) {
        if (message.startsWith(pattern)) {
            return message.replace(pattern, '').replace('_notification', '');
        }
    }
    return null;
}

function handleCallbackQuery(message, chatId, messageId, callbackUniqueId) {
    if (message.startsWith(`show_controls_`)) {
        ...
        showAFKReconnectMenu(chatId, messageId, callbackUniqueId, 'afk_n_fixed');
    } else if (message.startsWith(`afk_n_random_`)) {
        showAFKReconnectMenu(chatId, messageId, callbackUniqueId, 'afk_n_random');
    } else if (message.startsWith(`afk_n_without_pauses_`)) {
        showAFKReconnectMenu(chatId, messageId, callbackUniqueId, 'afk_n_without_pauses');
    } else if (message.startsWith(`afk_n_fixed_reconnect_on_`)) {
        showServerRestartActionMenu(chatId, messageId, callbackUniqueId, 'afk_n_fixed_reconnect_on_restart');
    } else if (message.startsWith(`afk_n_random_reconnect_on_`)) {
        showServerRestartActionMenu(chatId, messageId, callbackUniqueId, 'afk_n_random_reconnect_on_restart');
    } else if (message.startsWith(`afk_n_without_pauses_reconnect_on_`)) {
        showServerRestartActionMenu(chatId, messageId, callbackUniqueId, 'afk_n_without_pauses_reconnect_on_restart');
    } else if (message.startsWith(`afk_n_fixed_reconnect_off_`)) {
        activateAFKMode('fixed', false, 'q', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_random_reconnect_off_`)) {
        activateAFKMode('random', false, 'q', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_without_pauses_reconnect_off_`)) {
        activateAFKMode('none', false, 'q', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_fixed_reconnect_on_restart_q_`)) {
        activateAFKMode('fixed', true, 'q', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_fixed_reconnect_on_restart_rec_`)) {
        activateAFKMode('fixed', true, 'rec', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_random_reconnect_on_restart_q_`)) {
        activateAFKMode('random', true, 'q', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_random_reconnect_on_restart_rec_`)) {
        activateAFKMode('random', true, 'rec', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_without_pauses_reconnect_on_restart_q_`)) {
        activateAFKMode('none', true, 'q', chatId, messageId, callbackUniqueId);
    } else if (message.startsWith(`afk_n_without_pauses_reconnect_on_restart_rec_`)) {
        activateAFKMode('none', true, 'rec', chatId, messageId, callbackUniqueId);
        ...
        handleMovement('forward', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_back_")) {
        handleMovement('back', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_left_")) {
        handleMovement('left', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_right_")) {
        handleMovement('right', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_jump_")) {
        handleMovement('jump', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_punch_")) {
        handleMovement('punch', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_sit_")) {
        handleMovement('sit', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("move_stand_")) {
        handleMovement('stand', chatId, messageId, message.endsWith('_notification'));
    } else if (message.startsWith("back_to_notification_")) {
        const replyMarkup = {
            inline_keyboard: [
                [createButton("📝 Ответить", `admin_reply_${callbackUniqueId}`), createButton("🚶 Движения", `show_movement_${callbackUniqueId}`)]
            ]
        };
        editMessageReplyMarkup(chatId, messageId, replyMarkup);
...
// НОВАЯ ФУНКЦИЯ: Активация AFK режима с выбранными настройками
function activateAFKMode(mode, useReconnect, serverRestartAction, chatId, messageId, uniqueIdParam) {
    ...
    config.afkCycle.useReconnect = useReconnect;
    config.afkCycle.serverRestartAction = serverRestartAction;
    
    let modeText = '';
    if (mode === 'fixed') modeText = '5/5 минут';
    else if (mode === 'random') modeText = 'рандомное время';
    else if (mode === 'none') modeText = 'без пауз';
    
    const reconnectText = useReconnect ? '🟢 ВКЛ' : '🔴 ВЫКЛ';
    const restartActionText = serverRestartAction === 'q' ? '/q' : '/rec 5 (через 4 мин)';
    ...
    sendToTelegram(
        `🔄 <b>AFK режим активирован для ${displayName}</b>\n` +
        `ID из HUD: ${hudId}\n` +
        `Форматы: ${idFormats.join(', ')}\n` +
        `Режим: ${modeText}\n` +
        `Реконнект: ${reconnectText}\n` +
        `Действие при рестарте: ${restartActionText}${mode !== 'none' ? '\n🔁 <b>Запущен AFK цикл для PayDay</b>' : ''}`,
        false,
        null
    );
}

function handleMovement(direction, chatId, messageId, isNotif) {
    try {
        switch (direction) {
            case 'forward':
        ...
        const errorMsg = `❌ <b>Ошибка ${displayName}</b>\nНе удалось симулировать ${direction}\n<code>${err.message}</code>`;
        debugLog(errorMsg);
        sendToTelegram(errorMsg, false, null);
    }
}
...
// ОБНОВЛЕННАЯ ФУНКЦИЯ: Игровая фаза с поддержкой реконнекта
function startPlayPhase() {
    ...
    if (config.afkCycle.mode === 'fixed') {
        ...
        if (remainingPlay <= 0) {
            if (config.afkCycle.useReconnect) {
                enterReconnectMode();
            } else {
                enterPauseUntilEnd();
            }
    ...
    } else if (config.afkCycle.mode === 'none') {
        playDurationMs = 25 * 60 * 1000;
    }
    ...
    config.afkCycle.playTimer = setTimeout(() => {
        ...
        if (config.afkCycle.totalPlayTime < 25 * 60 * 1000) {
            ...
            debugLog(`Отыграно 25 минут для ${displayName}`);
            if (config.afkCycle.useReconnect) {
                sendToTelegram(`💤 <b>Отыграно 25 минут для ${displayName}</b>\nПереходим в режим реконнекта до следующего PayDay`, false, null);
                enterReconnectMode();
            } else {
...
// НОВАЯ ФУНКЦИЯ: Режим реконнекта
function enterReconnectMode() {
    debugLog(`Переход в режим реконнекта для ${displayName}`);
    
    // Отключаем автовход
    autoLoginConfig.enabled = false;
    sendToTelegram(`🔄 <b>Реконнект ${displayName}:</b> Отключён автовход`, false, null);
    
    // Отправляем /rec 5
    try {
        sendChatInput("/rec 5");
        sendToTelegram(`🔄 <b>Реконнект ${displayName}:</b> Отправлено /rec 5`, false, null);
    } catch (err) {
        debugLog(`Ошибка при отправке /rec 5: ${err.message}`);
    }
}

// ОБНОВЛЕННАЯ ФУНКЦИЯ: Обработка сообщения PayDay
function handlePayDayTimeMessage() {
    ...
    config.afkCycle.mainTimer = setTimeout(() => {
        // Включаем автовход перед следующим PayDay
        autoLoginConfig.enabled = true;
        sendToTelegram(`🔄 <b>Реконнект ${displayName}:</b> Включён автовход перед следующим PayDay`, false, null);
        
        // Отправляем второй /rec 5
        try {
            sendChatInput("/rec 5");
            sendToTelegram(`🔄 <b>Реконнект ${displayName}:</b> Отправлено второй /rec 5`, false, null);
        } catch (err) {
            debugLog(`Ошибка при отправке второго /rec 5: ${err.message}`);
        }
...
function initializeChatMonitor() {
    ...
    window.OnChatAddMessage = function(e, i, t) {
        ...
        // НОВАЯ ЛОГИКА: Обработка сообщения о возобновлении работы сервера
        if (config.afkSettings.active && config.afkCycle.active && msg.includes("Сервер возобновит работу в течение минуты...")) {
            ...
            if (config.afkCycle.serverRestartAction === 'q') {
                sendChatInput("/q");
                sendToTelegram(`⚡ <b>Автоматически отправлено /q (${displayName})</b>\nПо условию AFK ночь: Сервер возобновит работу`, false, null);
            } else if (config.afkCycle.serverRestartAction === 'rec') {
                sendToTelegram(`⏳ <b>Ожидание 4 минуты перед /rec 5 (${displayName})</b>\nПо условию AFK ночь: Сервер возобновит работу`, false, null);
                setTimeout(() => {
                    sendChatInput("/rec 5");
                    sendToTelegram(`⚡ <b>Автоматически отправлено /rec 5 (${displayName})</b>\nПрошло 4 минуты после предупреждения`, false, null);
                }, 4 * 60 * 1000); // 4 минуты
            }
        }
        ...
        if (lowerCaseMessage.includes("зареспавнил вас")) {
            ...
            const replyMarkup = {
                inline_keyboard: [
                    [createButton("📝 Ответить", `admin_reply_${uniqueId}`), createButton("🚶 Движения", `show_movement_${uniqueId}`)]
                ]
            };
        ...
        if ((lowerCaseMessage.indexOf("администратор") !== -1 && lowerCaseMessage.indexOf("для") !== -1) ||
            ...
            if (lowerCaseMessage.includes("подбросил")) {
                ...
                const replyMarkup = {
                    inline_keyboard: [
                        [createButton("📝 Ответить администратору", `admin_reply_${uniqueId}`), createButton("🚶 Движения", `show_movement_${uniqueId}`)]
                    ]
                };
        ...
        if (config.currentFaction && factions[config.currentFaction] && config.warningNotifications) {
            ...
            const rank9 = ranks[9];
            const escapedRank10 = rank10.replace(/[.*+?^${}()|[\]\\]/g, '\\    const authInstance = window.interface("Authorization");
    if (!authInstance) {
        debugLog(`Попытка ${attempt}: Экземпляр Authorization не найден, повтор через ${autoLoginConfig.attempt');
            const escapedRank9 = rank9.replace(/[.*+?^${}()|[\]\\]/g, '\\    const authInstance = window.interface("Authorization");
    if (!authInstance) {
        debugLog(`Попытка ${attempt}: Экземпляр Authorization не найден, повтор через ${autoLoginConfig.attempt');
