...
// Глобальная очередь команд для синхронизации между аккаунтами
const HB_COMMAND_QUEUE = {
    lastCommandId: 0,
    
    // Получить последний ID команды из localStorage
    getLastCommandId: function() {
        try {
            return parseInt(localStorage.getItem('hb_last_command_id') || '0', 10);
        } catch (e) {
            return 0;
        }
    },
    
    // Установить последний ID команды
    setLastCommandId: function(id) {
        try {
            localStorage.setItem('hb_last_command_id', id.toString());
        } catch (e) {
            debugLog(`Ошибка сохранения command ID: ${e.message}`);
        }
    },
    
    // Добавить команду в очередь
    addCommand: function(commandType, value) {
        try {
            const commandId = Date.now(); // Используем timestamp как уникальный ID
            const command = {
                id: commandId,
                type: commandType,
                value: value,
            ...
            localStorage.setItem('hb_command_' + commandId, JSON.stringify(command));
            this.setLastCommandId(commandId);
            
            debugLog(`[HB Queue] Добавлена команда: ${commandType} = ${value} (ID: ${commandId})`);
        } catch (e) {
            debugLog(`Ошибка добавления команды: ${e.message}`);
        }
    },
    
    // Проверить и обработать новые команды
    checkCommands: function() {
        try {
            const currentLastId = this.getLastCommandId();
            
            // Если есть новые команды
            if (currentLastId > this.lastCommandId) {
                // Ищем все команды с ID больше чем наш последний обработанный
                for (let i = 0; i < localStorage.length; i++) {
                    const key = localStorage.key(i);
                    
                    if (key && key.startsWith('hb_command_')) {
                        const commandId = parseInt(key.replace('hb_command_', ''), 10);
                        ...
                        if (commandId > this.lastCommandId) {
                            try {
                                ...
                                if (commandStr) {
                                    ...
                                    // Удаляем старую команду (старше 5 секунд)
                                    if (Date.now() - command.timestamp > 5000) {
                                        localStorage.removeItem(key);
                                    }
                                }
                            } catch (e) {
                                debugLog(`Ошибка обработки команды ${commandId}: ${e.message}`);
                            }
                        }
                    }
                }
                
                this.lastCommandId = currentLastId;
            }
        } catch (e) {
            debugLog(`Ошибка проверки команд: ${e.message}`);
        }
    },
    
    // Обработать команду
    processCommand: function(command) {
        debugLog(`[HB Queue] Обработка команды: ${command.type} = ${command.value}`);
        
        let updated = false;
        
        switch (command.type) {
            case 'paydayNotifications':
                if (config.paydayNotifications !== command.value) {
                    config.paydayNotifications = command.value;
                    updated = true;
                    debugLog(`[HB Queue] PayDay обновлен: ${command.value}`);
                }
                break;
                
            case 'govMessagesEnabled':
                if (config.govMessagesEnabled !== command.value) {
                    config.govMessagesEnabled = command.value;
                    updated = true;
                    debugLog(`[HB Queue] Сообщения обновлены: ${command.value}`);
                }
                break;
                
            case 'trackLocationRequests':
                if (config.trackLocationRequests !== command.value) {
                    config.trackLocationRequests = command.value;
                    updated = true;
                    debugLog(`[HB Queue] Отслеживание обновлено: ${command.value}`);
                }
                break;
                
            case 'radioOfficialNotifications':
                if (config.radioOfficialNotifications !== command.value) {
                    config.radioOfficialNotifications = command.value;
                    updated = true;
                    debugLog(`[HB Queue] Рация обновлена: ${command.value}`);
                }
                break;
                
            case 'warningNotifications':
                if (config.warningNotifications !== command.value) {
                    config.warningNotifications = command.value;
                    updated = true;
                    debugLog(`[HB Queue] Выговоры обновлены: ${command.value}`);
                }
                break;
        }
        
        // Обновляем welcome message если были изменения
        if (updated) {
            sendWelcomeMessage();
        }
    }
...
// Инициализация - установить начальный lastCommandId
HB_COMMAND_QUEUE.lastCommandId = HB_COMMAND_QUEUE.getLastCommandId();

// Постоянная проверка новых команд каждые 300мс
setInterval(() => {
    HB_COMMAND_QUEUE.checkCommands();
}, 300);
    ...
    switch (dialogId) {
        ...
        case HB_DIALOG_IDS.LOCAL_FUNCTIONS:
            if (listitem === 0) {
                ...
                sendToTelegram(`${config.govMessagesEnabled ? '🔔' : '🔕'} <b>Уведомления от сотрудников ${config.govMessagesEnabled ? 'включены' : 'отключены'} для ${displayName}</b>`, false, null);
                sendWelcomeMessage();
                setTimeout(() => showHBLocalFunctionsMenu(), 100);
            } else if (listitem === 3) {
                config.trackLocationRequests = !config.trackLocationRequests;
                sendToTelegram(`${config.trackLocationRequests ? '📍' : '🔕'} <b>Отслеживание ${config.trackLocationRequests ? 'включено' : 'отключено'} для ${displayName}</b>`, false, null);
                sendWelcomeMessage();
                setTimeout(() => showHBLocalFunctionsMenu(), 100);
            } else if (listitem === 4) {
                config.radioOfficialNotifications = !config.radioOfficialNotifications;
                sendToTelegram(`${config.radioOfficialNotifications ? '📡' : '🔕'} <b>Уведомления рации ${config.radioOfficialNotifications ? 'включены' : 'отключены'} для ${displayName}</b>`, false, null);
                sendWelcomeMessage();
                setTimeout(() => showHBLocalFunctionsMenu(), 100);
            } else if (listitem === 5) {
                config.warningNotifications = !config.warningNotifications;
                sendToTelegram(`${config.warningNotifications ? '⚠️' : '🔕'} <b>Уведомления выговоров ${config.warningNotifications ? 'включены' : 'отключены'} для ${displayName}</b>`, false, null);
                sendWelcomeMessage();
                setTimeout(() => showHBLocalFunctionsMenu(), 100);
            }
            break;
        ...
        case HB_DIALOG_IDS.GLOBAL_FUNCTIONS:
            if (listitem === 0) {
                ...
                const newPaydayValue = !config.paydayNotifications;
                HB_COMMAND_QUEUE.addCommand('paydayNotifications', newPaydayValue);
                ...
                const newSoobValue = !config.govMessagesEnabled;
                HB_COMMAND_QUEUE.addCommand('govMessagesEnabled', newSoobValue);
                sendToTelegram(`${newSoobValue ? '🔔' : '🔕'} <b>Сообщения от правительства ${newSoobValue ? 'включены' : 'отключены'} для ВСЕХ аккаунтов</b>`, false, null);
                setTimeout(() => showHBGlobalFunctionsMenu(), 100);
            } else if (listitem === 3) {
                // Место - ГЛОБАЛЬНАЯ настройка
                const newMestoValue = !config.trackLocationRequests;
                HB_COMMAND_QUEUE.addCommand('trackLocationRequests', newMestoValue);
                ...
                const newRadioValue = !config.radioOfficialNotifications;
                HB_COMMAND_QUEUE.addCommand('radioOfficialNotifications', newRadioValue);
                ...
                const newWarningValue = !config.warningNotifications;
                HB_COMMAND_QUEUE.addCommand('warningNotifications', newWarningValue);
...
// Сесть/Встать
                try {
                    ...
                    sendToTelegram(`✅ <b>Команда "${config.isSitting ? 'Сесть' : 'Встать'}" для ${displayName}</b>`, false, null);
                    setTimeout(() => showHBMovementMenu(), 100);
                } catch (err) {
                    sendToTelegram(`❌ <b>Ошибка:</b> ${err.message}`, false, null);
                }
...
console.log('[HB Menu] Проверка команд каждые 300мс');
