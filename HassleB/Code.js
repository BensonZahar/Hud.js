...
// ╔══════════════════════════════════════════════════════════╗
// ║  MODULE: TELEGRAM TEXT MOVEMENT                          ║
// ║  Описание: Управление персонажем через текстовые         ║
// ║             сообщения Telegram.                          ║
// ║             Работает без фокуса на окне игры.            ║
// ║                                                          ║
// ║  Команды (отправить в Telegram-чат):                     ║
// ║    /moveNick_Server w        — вперёд (непрерывно)       ║
// ║    /moveNick_Server w 3      — вперёд 3 секунды          ║
// ║    /moveNick_Server s        — назад                     ║
// ║    /moveNick_Server a        — влево                     ║
// ║    /moveNick_Server d        — вправо                    ║
// ║    /moveNick_Server stop     — остановить движение       ║
// ║    /moveNick_Server jump     — прыжок                    ║
// ║    /moveNick_Server sit      — сесть / встать            ║
// ║                                                          ║
// ║  ВАЖНО: джойстик регистрируется автоматически перед      ║
// ║         каждым вызовом — это ключ к работе без фокуса.  ║
// ║  Зависимости: debugLog                                   ║
// ╚══════════════════════════════════════════════════════════╝
// START TELEGRAM TEXT MOVEMENT MODULE //
...
const tmMove = {
    ...
    // ── Регистрируем виртуальный джойстик в движке ──────────
    // Без этого onScreenControlTouchStart/<Gamepad>/leftStick
    // игнорируется движком, особенно при отсутствии фокуса.
    _ensureJoystick() {
        try { window.onScreenJoystickCreate('<Gamepad>/leftStick'); } catch (e) {}
    },

    // ── Регистрируем кнопку прыжка ───────────────────────────
    _ensureJump() {
        try { window.onScreenButtonCreate('<Keyboard>/leftShift'); } catch (e) {}
    },

    // ── Начать непрерывное движение (durationMs=0 → до stop) ─
    start(x, y, durationMs) {
        this.stop();
        this._ensureJoystick();
        this.dirX = x;
        this.dirY = y;
        this.active = true;
        ...
        const doMove = () => {
            try {
                ...
                window.onScreenControlTouchMove('<Gamepad>/leftStick', this.dirX, this.dirY);
            } catch (e) {
                debugLog('[TMMOVE] Ошибка движения: ' + e.message);
            }
        };

        doMove();
        // Повторяем каждые 100 мс — движок не теряет ввод
        this.intervalId = setInterval(doMove, 100);

        if (durationMs > 0) {
            this.timeoutId = setTimeout(() => this.stop(), durationMs);
        }
        debugLog(`[TMMOVE] start x=${x} y=${y} dur=${durationMs}ms`);
    },
    ...
    stop() {
        ...
        if (this.active) {
            try { window.onScreenControlTouchEnd('<Gamepad>/leftStick'); } catch (e) {}
            this.active = false;
        }
        this.dirX = 0;
        this.dirY = 0;
        debugLog('[TMMOVE] stop');
    },
    ...
    jump() {
        ...
        try {
            ...
            setTimeout(() => {
                try { window.onScreenControlTouchEnd('<Keyboard>/leftShift'); } catch (e) {}
            }, 200);
            debugLog('[TMMOVE] jump');
        } catch (e) {
            debugLog('[TMMOVE] Ошибка прыжка: ' + e.message);
        }
    },

    // ── Сесть / Встать ────────────────────────────────────────
    sit() {
        try { window.onScreenButtonCreate('<Keyboard>/c'); } catch (e) {}
        try {
            window.onScreenControlTouchStart('<Keyboard>/c');
            setTimeout(() => {
                try { window.onScreenControlTouchEnd('<Keyboard>/c'); } catch (e) {}
            }, 300);
            debugLog('[TMMOVE] sit/stand');
        } catch (e) {
            debugLog('[TMMOVE] Ошибка sit/stand: ' + e.message);
        }
    }
};

// Парсит аргумент текстовой команды движения.
// Возвращает { action, x, y, duration } или null.
function parseTmMoveArg(arg) {
    if (!arg) return null;
    const t = arg.trim().toLowerCase();

    if (t === 'stop'  || t === 'стоп')   return { action: 'stop' };
    if (t === 'jump'  || t === 'прыжок' || t === 'j') return { action: 'jump' };
    if (t === 'sit'   || t === 'сесть'  || t === 'встать' || t === 'stand')
        return { action: 'sit' };

    const DIRS = {
        'w': [0, 1],   'вп': [0, 1],    'вперед': [0, 1],   'вперёд': [0, 1], 'forward': [0, 1],
        's': [0, -1],  'назад': [0, -1], 'back': [0, -1],    'backward': [0, -1],
        'a': [-1, 0],  'влево': [-1, 0], 'left': [-1, 0],
        'd': [1, 0],   'вправо': [1, 0], 'right': [1, 0],
    };
    ...
    for (const k of Object.keys(DIRS)) {
        ...
        // "w3" → key=w, duration=3s
        if (parts[0].startsWith(k)) {
            const suffix = parts[0].slice(k.length);
            if (suffix !== '' && !isNaN(suffix) && suffix.length > 0) {
                dirKey = k;
                duration = parseFloat(suffix) * 1000;
                break;
            }
        }
    }
    ...
    const [x, y] = DIRS[dirKey];

    // "w 3" — число после пробела
    if (!duration && parts.length > 1 && !isNaN(parts[1]) && parts[1] !== '') {
        duration = parseFloat(parts[1]) * 1000;
    }

    return { action: 'move', x, y, duration };
}

const TM_DIR_LABELS = {
    '0,1': '⬆️ вперёд', '0,-1': '⬇️ назад',
    '-1,0': '⬅️ влево',  '1,0': '➡️ вправо',
};
// END TELEGRAM TEXT MOVEMENT MODULE //
