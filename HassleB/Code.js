        ...
        if (update.message) {
            ...
            if (!update.message.reply_to_message && message) {
                const pendingKey = `${chatId}_${uniqueId}`;
				        // [DIALOG MODULE] iOS fix для ввода текста в диалог
				if (handleDialogPendingInput(chatId, message)) {
					continue;
				}
            ...
            if (update.message.reply_to_message) {
				...
				if (handleDialogTextInput(replyToText, message, chatId)) {
                    continue;
                }
			...
			} else if (message.startsWith('dlg_')) {
                callbackUniqueId = parseDialogCallbackUniqueId(message);
            }
			...
			// [DIALOG MODULE] Обработка кнопок диалогового окна
            if (message.startsWith('dlg_')) {
                // handleDialogCallback сам вызывает answerCallbackQuery
                handleDialogCallback(message, chatId, messageId, callbackQueryId);
                continue; // пропускаем answerCallbackQuery ниже
            }
...
// Управление диалоговыми окнами игры через Telegram
// Поддерживает: Window (текстовые окна) и InputWindow (окна с полем ввода)
//
// Команды в Telegram:
//   Кнопки: 🔘 Кнопка 1 (Enter) / ❌ Кнопка 2 (Esc) / ❌ Закрыть
//   Ввод текста: reply на запрос ввода → текст вставляется в поле и подтверждается
// ====================================================================

// --- Состояние активного диалога ---
const dialogState = {
    active: false,          // Открыт ли сейчас диалог
    dialogId: null,         // ID диалога (из showDialog)
    type: null,             // 'window' | 'input'
    title: '',              // Заголовок
    buttons: [],            // ['Принять', 'Отмена'] и т.п.
    hasInput: false,        // Есть ли поле ввода
    tgMessageId: null,      // ID сообщения в Telegram (для редактирования)
    pendingInput: false,    // Ожидаем ввод текста из Telegram
    pendingInputTimestamp: 0
};
const DIALOG_INPUT_TTL = 60 * 1000; // 60 секунд на ввод

// --- Перехват showDialog для отслеживания открытия окон ---
const _originalShowDialog = window.showDialog;
window.showDialog = function(dialogId, type, title, subtitle, text, buttons, ...rest) {
    // Вызываем оригинал
    let result;
    if (typeof _originalShowDialog === 'function') {
        result = _originalShowDialog.call(this, dialogId, type, title, subtitle, text, buttons, ...rest);
    }

    try {
        // type: 0 = MSGBOX, 1 = INPUT, 2 = LIST, 3 = PASSWORD, 4+ = другие
        // Window.js: текстовое окно (type 0/4/5) и input (type 1/3)
        const isInput = (type === 1 || type === 3);
        dialogState.active = true;
        dialogState.dialogId = dialogId;
        dialogState.type = isInput ? 'input' : 'window';
        dialogState.hasInput = isInput;
        dialogState.title = title || '';
        dialogState.buttons = Array.isArray(buttons) ? buttons : (buttons ? [buttons] : ['OK']);
        dialogState.pendingInput = false;

        debugLog(`[Dialog] Открыт диалог ID=${dialogId}, type=${type}, title="${title}"`);

        // Уведомляем в Telegram
        notifyDialogOpen();
    } catch (e) {
        debugLog(`[Dialog] Ошибка при перехвате showDialog: ${e.message}`);
    }

    return result;
};

// --- Перехват sendClientEvent для отслеживания закрытия диалогов ---
// Диалог закрывается когда уходит OnDialogResponse с нашим dialogId
const _origSendClientEventForDialog = window.sendClientEventCustom || window.sendClientEvent;
const _dialogInterceptWrapper = function(event, ...args) {
    if (event === "OnDialogResponse" || args[0] === "OnDialogResponse") {
        const dId = (event === "OnDialogResponse") ? args[0] : args[1];
        if (dId === dialogState.dialogId && dialogState.active) {
            dialogState.active = false;
            dialogState.tgMessageId = null;
            dialogState.pendingInput = false;
            debugLog(`[Dialog] Диалог ID=${dId} закрыт через OnDialogResponse`);
        }
    }
    // Передаём дальше — wrapper из HB Menu уже стоит
    if (typeof _origSendClientEventForDialog === 'function') {
        return _origSendClientEventForDialog(event, ...args);
    }
};
// Не перезаписываем sendClientEventCustom — HB меню уже управляет им,
// поэтому просто подписываемся через флаг (см. обработку в processUpdates ниже)

// --- Отправка уведомления об открытом диалоге в Telegram ---
function notifyDialogOpen() {
    if (!config.botToken || !config.chatIds || !config.chatIds.length) return;

    const typeEmoji = dialogState.hasInput ? '📝' : '🗔';
    let text = `${typeEmoji} <b>Открылось диалоговое окно (${displayName})</b>\n`;

    if (dialogState.title) {
        text += `📌 <b>${dialogState.title.replace(/</g, '&lt;')}</b>\n`;
    }

    if (dialogState.hasInput) {
        text += `\n✏️ <i>Это окно с полем ввода.</i>\nНажмите кнопку ниже, чтобы ввести текст через Telegram.`;
    }

    // Формируем кнопки
    const keyboard = buildDialogKeyboard();
    ...
    config.chatIds.forEach(chatId => {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                try {
                    const data = JSON.parse(xhr.responseText);
                    dialogState.tgMessageId = data.result.message_id;
                    debugLog(`[Dialog] Уведомление отправлено, msg_id=${dialogState.tgMessageId}`);
                } catch(e) {}
            }
        };
        xhr.send(JSON.stringify(payload));
...
// --- Строим inline-клавиатуру для диалога ---
function buildDialogKeyboard() {
    const btns = dialogState.buttons;
    const rows = [];

    // Основные кнопки диалога (Enter = кнопка 0, Esc = кнопка 1)
    const actionRow = [];
    if (btns[0]) {
        actionRow.push(createButton(`✅ ${btns[0]}`, `dlg_btn0_${uniqueId}`));
    }
    if (btns[1] && btns[1] !== '') {
        actionRow.push(createButton(`❌ ${btns[1]}`, `dlg_btn1_${uniqueId}`));
    }
    if (actionRow.length) rows.push(actionRow);

    // Кнопка ввода текста — только если есть поле ввода
    if (dialogState.hasInput) {
        rows.push([createButton('✏️ Ввести текст', `dlg_input_${uniqueId}`)]);
    }

    // Кнопка принудительного закрытия (ESC)
    rows.push([createButton('🚫 Закрыть (Esc)', `dlg_close_${uniqueId}`)]);

    return { inline_keyboard: rows };
}

// --- Нажать кнопку диалога (button=0 Enter, button=1 Esc) ---
function pressDialogButton(buttonIndex) {
    if (!dialogState.active || dialogState.dialogId === null) {
        debugLog(`[Dialog] pressDialogButton: нет активного диалога`);
        return false;
    }
    try {
        // Эмулируем OnDialogResponse: (dialogId, button, listitem, inputText)
        sendClientEvent("OnDialogResponse", dialogState.dialogId, buttonIndex, 0, "");
        debugLog(`[Dialog] Нажата кнопка ${buttonIndex} в диалоге ID=${dialogState.dialogId}`);
        dialogState.active = false;
        return true;
    } catch (e) {
        debugLog(`[Dialog] Ошибка pressDialogButton: ${e.message}`);
        return false;
    }
}

// --- Нажать кнопку с вводом текста ---
function submitDialogInput(inputText) {
    if (!dialogState.active || dialogState.dialogId === null) {
        debugLog(`[Dialog] submitDialogInput: нет активного диалога`);
        return false;
    }
    try {
        // Для input-диалога: button=0 (Enter), listitem=0, inputText = текст
        sendClientEvent("OnDialogResponse", dialogState.dialogId, 0, 0, inputText || "");
        debugLog(`[Dialog] Отправлен ввод "${inputText}" в диалог ID=${dialogState.dialogId}`);
        dialogState.active = false;
        return true;
    } catch (e) {
        debugLog(`[Dialog] Ошибка submitDialogInput: ${e.message}`);
        return false;
    }
}

// --- Обновить сообщение в Telegram (закрыть кнопки после действия) ---
function updateDialogTgMessage(resultText) {
    if (!dialogState.tgMessageId) return;
    config.chatIds.forEach(chatId => {
        const url = `https://api.telegram.org/bot${config.botToken}/editMessageText`;
        const payload = {
            chat_id: chatId,
            message_id: dialogState.tgMessageId,
            text: resultText,
            parse_mode: 'HTML'
        };
        const xhr = new XMLHttpRequest();
...
// ==================== ИНТЕГРАЦИЯ В processUpdates ====================
// Добавляем обработку callback_query для диалогов.
// Вставляется в блок "else if" цепочки callback_query обработчиков.
// ВАЖНО: этот код нужно добавить ВНУТРЬ существующего блока processUpdates,
// в секцию callback_query, перед строкой "answerCallbackQuery(callbackQueryId);"

// Регистрируем префиксы для callbackUniqueId парсинга
// (добавляем в существующий if-else каскад parseCallbackUniqueId)
function parseDialogCallbackUniqueId(msg) {
    if (msg.startsWith('dlg_btn0_')) return msg.replace('dlg_btn0_', '');
    if (msg.startsWith('dlg_btn1_')) return msg.replace('dlg_btn1_', '');
    if (msg.startsWith('dlg_input_')) return msg.replace('dlg_input_', '');
    if (msg.startsWith('dlg_close_')) return msg.replace('dlg_close_', '');
    if (msg.startsWith('dlg_text_confirm_')) return msg.replace('dlg_text_confirm_', '');
    return null;
}

// Обработчик dialog callback — вызывается из processUpdates
function handleDialogCallback(msg, chatId, messageId, callbackQueryId) {
    if (!msg.startsWith('dlg_')) return false; // не наш callback

    const parsedId = parseDialogCallbackUniqueId(msg);
    if (!parsedId || parsedId !== uniqueId) return false; // не для этого аккаунта

    if (msg.startsWith('dlg_btn0_')) {
        // --- Кнопка 0: Enter ---
        if (!dialogState.active) {
            editMessageTextSimple(chatId, messageId, `ℹ️ <b>Диалог уже закрыт (${displayName})</b>`);
            answerCallbackQuery(callbackQueryId);
            return true;
        }
        const btnName = dialogState.buttons[0] || 'OK';
        const ok = pressDialogButton(0);
        if (ok) {
            updateDialogTgMessage(`✅ <b>Нажата кнопка "${btnName}" (${displayName})</b>`);
            sendToTelegram(`✅ <b>Диалог: нажата кнопка "${btnName}" (${displayName})</b>`, true, null);
        } else {
            editMessageTextSimple(chatId, messageId, `❌ <b>Не удалось нажать кнопку (${displayName})</b>`);
        }

    } else if (msg.startsWith('dlg_btn1_')) {
        // --- Кнопка 1: Esc ---
        if (!dialogState.active) {
            editMessageTextSimple(chatId, messageId, `ℹ️ <b>Диалог уже закрыт (${displayName})</b>`);
            answerCallbackQuery(callbackQueryId);
        ...
        const btnName = dialogState.buttons[1] || 'Отмена';
        const ok = pressDialogButton(1);
        if (ok) {
            updateDialogTgMessage(`❌ <b>Нажата кнопка "${btnName}" (${displayName})</b>`);
            sendToTelegram(`❌ <b>Диалог: нажата кнопка "${btnName}" (${displayName})</b>`, true, null);
        } else {
    ...
    } else if (msg.startsWith('dlg_input_')) {
        // --- Запрос ввода текста ---
        if (!dialogState.active) {
            editMessageTextSimple(chatId, messageId, `ℹ️ <b>Диалог уже закрыт (${displayName})</b>`);
            answerCallbackQuery(callbackQueryId);
        ...
        // Ставим флаг ожидания ввода
        dialogState.pendingInput = true;
        dialogState.pendingInputTimestamp = Date.now();

        const requestMsg = `✏️ <b>Введите текст для диалога (${displayName}):</b>\n📌 <i>${dialogState.title.replace(/</g, '&lt;')}</i>\n🔑 ID: dlg_${uniqueId}`;
        // iOS fix: сохраняем в pendingInputs
        config.chatIds.forEach(cId => {
            pendingInputs[`${cId}_dlg_${uniqueId}`] = { type: 'dialog_input', timestamp: Date.now() };
        });
        sendToTelegram(requestMsg, false, { force_reply: true });

    } else if (msg.startsWith('dlg_close_')) {
        // --- Принудительное закрытие (ESC) ---
        if (!dialogState.active) {
            editMessageTextSimple(chatId, messageId, `ℹ️ <b>Диалог уже закрыт (${displayName})</b>`);
            answerCallbackQuery(callbackQueryId);
            return true;
        }
        const ok = pressDialogButton(1); // 1 = Esc
        if (ok) {
            updateDialogTgMessage(`🚫 <b>Диалог закрыт (Esc) (${displayName})</b>`);
        } else {
            editMessageTextSimple(chatId, messageId, `❌ <b>Не удалось закрыть диалог (${displayName})</b>`);
        }
    }
...
// Обработка reply-сообщений с текстом для диалога — вызывается из processUpdates
function handleDialogTextInput(replyToText, messageText, chatId) {
    // Проверяем что это reply для диалога
    if (!replyToText.includes(`dlg_${uniqueId}`)) return false;
    if (!dialogState.active || !dialogState.hasInput) {
        sendToTelegram(`ℹ️ <b>Диалог уже закрыт или не требует ввода (${displayName})</b>`, true, null);
        return true;
    }
    // Сбрасываем pending
    delete pendingInputs[`${chatId}_dlg_${uniqueId}`];
    dialogState.pendingInput = false;

    const ok = submitDialogInput(messageText);
    if (ok) {
        updateDialogTgMessage(`✅ <b>Текст отправлен в диалог (${displayName}):</b>\n<code>${messageText.replace(/</g, '&lt;')}</code>`);
        sendToTelegram(`✅ <b>Диалог: текст введён (${displayName}):</b>\n<code>${messageText.replace(/</g, '&lt;')}</code>`, true, null);
    } else {
        sendToTelegram(`❌ <b>Не удалось ввести текст в диалог (${displayName})</b>`, false, null);
    }
    return true;
}

// iOS fix: обработка pendingInputs для dialog_input без reply
function handleDialogPendingInput(chatId, messageText) {
    const pendingKey = `${chatId}_dlg_${uniqueId}`;
    const pending = pendingInputs[pendingKey];
    if (!pending || pending.type !== 'dialog_input') return false;
    if (Date.now() - pending.timestamp >= DIALOG_INPUT_TTL) {
        delete pendingInputs[pendingKey];
        return false;
    }
    delete pendingInputs[pendingKey];
    ...
    if (!dialogState.active || !dialogState.hasInput) {
        sendToTelegram(`ℹ️ <b>Диалог уже закрыт (${displayName})</b>`, true, null);
        return true;
    }
...
// Вспомогательная функция для редактирования текста сообщения
function editMessageTextSimple(chatId, messageId, text) {
    const url = `https://api.telegram.org/bot${config.botToken}/editMessageText`;
    const payload = {
        chat_id: chatId,
        message_id: messageId,
...
debugLog('[Dialog Module] Модуль управления диалогами загружен');
