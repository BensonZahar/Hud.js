...
// └──────────────────────────────────────────────────────────┘
const BOT_NAME = 'Hassle | BotАвтошк099оа'; // Имя бота в приветственном сообщении
...
// ╔══════════════════════════════════════════════════════════╗
// ║  MODULE: АВТОШКОЛА v5                                    ║
// ║  FIX: OnScreenControlSetValue для полного нажатия        ║
// ║  FIX: Убран TouchMove (ломал ввод)                       ║
// ║  FIX: Повторный TouchStart каждые 200мс для удержания   ║
// ║  + Полное логирование с координатами                     ║
// ╚══════════════════════════════════════════════════════════╝
(function () {
'use strict';
...
// ═══ v5: интервал повторного TouchStart для удержания ═══
const HOLD_REPEAT_INTERVAL  = 200; // мс
...
const avto = {
    ...
    detailedLog: [],
    replayDetailedLog: [],
    // ═══ v5: таймеры удержания ═══
    holdTimers: {},   // keyCode → intervalId
};

// ═══════════════════════════════════════════════════════════
// ── v5: ПРАВИЛЬНЫЙ ввод для мобильного движка ─────────────
// ═══════════════════════════════════════════════════════════

function _inputDown(path, code) {
    // 1. TouchStart — сообщает движку о начале нажатия
    window.onScreenControlTouchStart(path);
    // 2. SetValue(1.0) — гарантирует ПОЛНОЕ нажатие (газ/руль на 100%)
    if (typeof window.onScreenControlSetValue === 'function') {
        window.onScreenControlSetValue(path, 1.0);
    }
    // 3. Повторный TouchStart каждые HOLD_REPEAT_INTERVAL мс
    //    (некоторые движки сбрасывают состояние без повторного сигнала)
    _holdStop(code);
    avto.holdTimers[code] = setInterval(function() {
        try {
            window.onScreenControlTouchStart(path);
            if (typeof window.onScreenControlSetValue === 'function') {
                window.onScreenControlSetValue(path, 1.0);
...
function _inputUp(path, code) {
    _holdStop(code);
    // 1. SetValue(0.0) — отпускаем
    if (typeof window.onScreenControlSetValue === 'function') {
        window.onScreenControlSetValue(path, 0.0);
    }
    // 2. TouchEnd — завершаем
    window.onScreenControlTouchEnd(path);
}

function _holdStop(code) {
    if (avto.holdTimers[code]) {
        clearInterval(avto.holdTimers[code]);
        delete avto.holdTimers[code];
    }
}
        ...
        if (action === 'UP') {
            for (var i = avto.detailedLog.length - 1; i >= 0; i--) {
                if (avto.detailedLog[i].keyCode === code && avto.detailedLog[i].action === 'DOWN') {
                    entry.duration = entry.t - avto.detailedLog[i].t;
                    break;
                }
            }
        }
...
// ═══════════════════════════════════════════════════════════
// ── ПОВТОР (v5: SetValue + повторный TouchStart) ──────────
// ═══════════════════════════════════════════════════════════
...
function replayRoute() {
    ...
    var inputMethod = hasTouchControl
        ? 'touch+SetValue(' + (hasSetValue ? '✅' : '❌') + ') v5'
        : 'keyboard';
    _chat('{33DD77}▶ Повтор (' + (totalMs/1000).toFixed(2) + 'с) | Метод: ' + inputMethod);
    _chat('{AAAAAA}🔧 HoldRepeat: ' + HOLD_REPEAT_INTERVAL + 'мс');
    debugLog('[АВТОШКОЛА] Повтор. Метод=' + inputMethod);
    ...
    function _execEvent(ev, elapsed) {
        ...
        try {
            if (ev.d === 1) {
                if (hasTouchControl) {
                    _inputDown(info.path, ev.k);  // ← v5: TouchStart + SetValue(1.0) + repeat
                } else {
                ...
                if (hasTouchControl) {
                    _inputUp(info.path, ev.k);  // ← v5: SetValue(0.0) + TouchEnd
                } else {
...
function _sendRecordingToTelegram(totalSec) {
    ...
    var msg = '🏎 <b>АВТОШКОЛА v5 — Маршрут записан</b>\n\n';
    ...
    msg += '📍 Старт: <b>' + _fmtPos(snap?.pos) + '</b> ∠' + _fmtAngle(snap?.pos?.angle) + '\n';
    msg += '🏁 Финиш: <b>' + _fmtPos(endSnap?.pos) + '</b> ∠' + _fmtAngle(endSnap?.pos?.angle) + '\n\n';

    // Детальный лог
    msg += '📋 <b>ДЕТАЛЬНЫЙ ЛОГ КЛАВИШ (запись):</b>\n<code>\n';
    var evNum = 0;
    avto.detailedLog.forEach(function(entry) {
        evNum++;
        var arrow = entry.action === 'DOWN' ? '⬇ нажата' : '⬆ отпущена';
        var line = '#' + String(evNum).padStart(3,'0') + ' ' + (entry.t/1000).toFixed(3) + 'с ' + arrow + ' ' + entry.key;
        ...
        if (entry.posBefore && entry.posAfter) {
            ...
            if (d !== null) line += '\n   📏 пройдено за событие: ' + d.toFixed(2) + 'м';
        }
        msg += line + '\n';
    });
    msg += '</code>';
    ...
    if (msg.length <= MAX_LEN) {
        ...
        var splitIdx = msg.indexOf('<code>');
        if (splitIdx > 0 && splitIdx < MAX_LEN) {
            sendToTelegram(msg.substring(0, splitIdx), false, null);
            var body = msg.substring(splitIdx);
            for (var i = 0; i < body.length; i += 3500) {
                sendToTelegram(body.substring(i, i + 3500), true, null);
            }
        } else {
            for (var i = 0; i < msg.length; i += MAX_LEN) {
                sendToTelegram(msg.substring(i, i + MAX_LEN), i > 0, null);
            }
        }
    }
    ...
    for (var i = 0; i < routeJson.length; i += 3800) {
        ...
        sendToTelegram('📦 Данные маршрута:\n<code>' + chunk + '</code>', true, null);
    }
}
...
function _sendReplayResultToTelegram(stats, endMsgs, inputMethod) {
    ...
    var msg = '🏁 <b>АВТОШКОЛА v5 — Повтор завершён</b>\n\n';
    msg += '🎮 Метод: <b>' + inputMethod + '</b>\n';
    msg += '🔧 HoldRepeat: <b>' + HOLD_REPEAT_INTERVAL + 'мс</b>\n';
    ...
    if (msg.length <= 4000) {
        ...
        for (var i = 0; i < msg.length; i += 3800) {
            sendToTelegram(msg.substring(i, i + 3800), i > 0, null);
        }
    }
}
...
_chat('{AAAAAA}v5 загружен | SetValue+HoldRepeat | /arec_on /arec_off /apov /apov_off');
debugLog('[АВТОШКОЛА v5] SetValue=' + (typeof window.onScreenControlSetValue === 'function') + ' HoldRepeat=' + HOLD_REPEAT_INTERVAL + 'мс');
})();
