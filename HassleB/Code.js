// Глобальные константы и переменные (доступны всем модулям)
const SERVER_TOKENS = {
...
(function() {
    ...
    // Экспорт, если нужно (но в коде используется глобально)
    window.normalizeColor = normalizeColor;
    window.getChatRadius = getChatRadius;
})();
...
(function() {
    ...
    window.autoLoginConfig = autoLoginConfig; // Экспорт для других модулей, если нужно
})();
...
(function() {
    ...
    // Экспорт
    window.getSharedLastUpdateId = getSharedLastUpdateId;
    window.setSharedLastUpdateId = setSharedLastUpdateId;
})();

// DEBUG AND UTILS MODULE
(function() {
    ...
    window.normalizeToCyrillic = normalizeToCyrillic;
    window.showScreenNotification = showScreenNotification;
})();
...
(function() {
    ...
    window.editMessageReplyMarkup = editMessageReplyMarkup;
    window.editMessageText = editMessageText;
    window.answerCallbackQuery = answerCallbackQuery;
})();
...
(function() {
    ...
    // Экспорт функции активации (если используется в меню)
    window.activateAFKWithMode = activateAFKWithMode;
    window.handlePayDayTimeMessage = handlePayDayTimeMessage; // Для вызова из чат-монитора
})();
...
(function() {
    ...
    // Экспорт
    window.showControlsMenu = showControlsMenu;
    window.showGlobalFunctionsMenu = showGlobalFunctionsMenu;
    // ... и другие, если используются вне
})();
...
(function() {
    ...
    // Самозапуск проверки команд
    checkTelegramCommands();
    // Экспорт processUpdates (если используется в других IIFE, например, в DIALOG)
...
(function() {
    ...
    // Экспорт функций для чат-монитора
    window.isNonRPMessage = isNonRPMessage;
    window.checkIDFormats = checkIDFormats;
    window.getRankKeywords = getRankKeywords;
    window.getHighRankKeywords = getHighRankKeywords;
    window.checkRoleAndActionConditions = checkRoleAndActionConditions;
    window.checkAFKConditions = checkAFKConditions;
    window.checkLocationRequest = checkLocationRequest;
window.isTargetingPlayer = isTargetingPlayer;
    window.processSalaryAndBalance = processSalaryAndBalance;
    window.checkGovMessageConditions = checkGovMessageConditions;
})();
    ...
    function initializeChatMonitor() {
        ...
        window.OnChatAddMessage = function(e, i, t) {
            ...
            if (!isNonRPMessage(msg) && getHighRankKeywords().some(kw => lowerCaseMessage.includes(kw)) &&
                ...
                // Извлекаем ник отправителя из сообщения
            ...
            if (!isNonRPMessage(msg) && checkLocationRequest(msg, lowerCaseMessage, chatRadius)) {
                ...
                sendToTelegram(`📍 <b>Обнаружен запрос местоположения (${displayName}) (${displayName}):</b>\n<code>${msg.replace(/</g, '&lt;')}</code>`, false, replyMarkup);
            }
...
(function() {
    ...
    // Экспорт
    window.performReconnect = performReconnect;
})();
...
(function() {
    // Новая переменная для хранения состояния диалогов (локально в модуле)
...
(function() {
    ...
    function showHBMovementMenu() {
        ...
        const menuItems = [
            ...
            { name: "{FFFFFF}Назад", action: "move_back" },
