...
(function () {
    ...
    const avto = {
        ...
        // ── Детальный лог нажатий с позицией (запись) ─────────
        detailedLog:      [],      // [{t, k, d, pos, inVehicle}] — снимок на каждое нажатие/отпускание
        lastDetailedLog:  null,    // детальный лог после остановки записи
        // ── Детальный лог нажатий с позицией (повтор) ─────────
        replayDetailedLog: [],     // [{t, k, d, pos, inVehicle}] — снимок на каждое воспроизведённое событие
    };
    ...
    function onKeyDown(e) {
        ...
        avto.detailedLog.push({ t: t, k: code, d: 1, pos: snap.pos, inVehicle: snap.inVehicle });
        debugLog('[АВТОШКОЛА] ⬇ ' + KEY_MAP[code].label + ' @ ' + t.toFixed(3) + 'мс | ' + _fmtPos(snap.pos));
    }
    ...
    function onKeyUp(e) {
        ...
        var snap = _getSnapshot();
        avto.detailedLog.push({ t: t, k: code, d: 0, pos: snap.pos, inVehicle: snap.inVehicle });
        debugLog('[АВТОШКОЛА] ⬆ ' + KEY_MAP[code].label + ' @ ' + t.toFixed(3) + 'мс | ' + _fmtPos(snap.pos));
    }
    ...
    function stopRecording() {
        ...
        avto.heldKeys.forEach(function (code) {
            var _t    = performance.now() - avto.startPerf;
            var _snap = _getSnapshot();
            avto.events.push({ t: _t, d: 0, k: code });
            avto.detailedLog.push({ t: _t, k: code, d: 0, pos: _snap.pos, inVehicle: _snap.inVehicle, forced: true });
            _overlaySetKey(code, false);
        });
        avto.heldKeys.clear();
        ...
        avto.lastDetailedLog = avto.detailedLog.slice();
        ...
        _sendDetailedLogToTelegram('record', avto.lastDetailedLog, null);
    }
    ...
    function replayRoute() {
        ...
        function _execEvent(ev, idx) {
            const info = KEY_MAP[ev.k];
            if (!info) return;
            var snapBefore = _getSnapshot();
            var actualT    = performance.now() - startPerf;
            try {
                if (ev.d === 1) {
                    if (hasTouchControl) {
                    ...
                    debugLog('[АВТОШКОЛА] ▶⬇ ' + info.label + ' | ' + _fmtPos(snapBefore.pos));
                } else {
                    if (hasTouchControl) {
            ...
            var snapAfter = _getSnapshot();
            avto.replayDetailedLog.push({
                idx:       idx,
                t:         ev.t,           // ожидаемое время (из записи)
                actualT:   actualT,        // фактическое время срабатывания
                k:         ev.k,
                d:         ev.d,
                posBefore: snapBefore.pos, // позиция непосредственно ДО срабатывания события
                posAfter:  snapAfter.pos,  // позиция непосредственно ПОСЛЕ срабатывания события
                inVehicle: snapAfter.inVehicle
            });
        }
        ...
        function _rafLoop() {
            ...
            if (elapsed < totalMs + 400) {
                ...
                _sendDetailedLogToTelegram('replay', avto.replayDetailedLog, avto.lastDetailedLog);
            }
        }
    ...
    // ── Вспомогательное: дополнение числа нулями до 3 знаков ──
    function _pad3(n) {
        var s = String(n);
    ...
    // ── Отправка ПОЛНОГО детального лога (каждое нажатие +     ──
    // ── позиция до/после) в Telegram — для запись И повтор     ──
    function _sendDetailedLogToTelegram(kind, log, refLog) {
        if (typeof sendToTelegram !== 'function') {
            debugLog('[АВТОШКОЛА] sendToTelegram недоступна, детальный лог не отправлен');
            return;
        }
        if (!log || log.length === 0) return;

        var isReplay  = (kind === 'replay');
        var titleBase = isReplay
            ? '🧾 <b>АВТОШКОЛА — Детальный лог ПОВТОРА</b>'
            : '🧾 <b>АВТОШКОЛА — Детальный лог ЗАПИСИ</b>';

        var lines = [];
        for (var i = 0; i < log.length; i++) {
            var e   = log[i];
            var lbl = KEY_MAP[e.k] ? KEY_MAP[e.k].label : ('Key' + e.k);
            var dir = e.d === 1 ? '⬇ нажата' : '⬆ отпущена';
            var num = '#' + _pad3(i + 1);

            if (!isReplay) {
                // ── Запись: время + позиция в момент события ──────
                var tSec     = (e.t / 1000).toFixed(3);
                var angleStr = (e.pos && e.pos.angle != null)
                    ? ' ∠' + e.pos.angle.toFixed(1) + '°' : '';
                lines.push(num + ' ' + dir + ' <b>' + lbl + '</b> @ ' + tSec + 'с' +
                           (e.forced ? ' (авто-отпуск при остановке)' : ''));
                lines.push('     📍 ' + _fmtPos(e.pos) + angleStr);
            } else {
                // ── Повтор: ожид./факт. тайминг + позиция до/после
                // ── + сравнение с позицией из записи (если есть) ──
                var expT  = (e.t / 1000).toFixed(3);
                var actT  = (e.actualT / 1000).toFixed(3);
                var drift = (e.actualT - e.t).toFixed(1);
                var ref   = (refLog && refLog[i]) ? refLog[i] : null;

                lines.push(num + ' ' + dir + ' <b>' + lbl + '</b>' +
                           ' | ожид=' + expT + 'с факт=' + actT + 'с (Δ' + drift + 'мс)');
                lines.push('     📍 до: '    + _fmtPos(e.posBefore));
                lines.push('     📍 после: ' + _fmtPos(e.posAfter));
                if (ref && ref.pos && e.posAfter) {
                    var d = _posDistance(ref.pos, e.posAfter);
                    if (d !== null) {
                        lines.push('     🆚 запись: ' + _fmtPos(ref.pos) +
                                   ' | расхождение: <b>' + d.toFixed(2) + 'м</b>');
                    }
                }
            }
        }

        // ── Разбиваем построчно на чанки, не разрывая теги/строки ─
        var chunkLimit = 3500;
        var chunks = [];
        var buf = '';
        for (var j = 0; j < lines.length; j++) {
            var ln = lines[j];
            if (buf.length + ln.length + 1 > chunkLimit) {
                chunks.push(buf);
                buf = ln;
            } else {
                buf = buf ? (buf + '\n' + ln) : ln;
        ...
        for (var c = 0; c < chunks.length; c++) {
            var part   = chunks.length > 1 ? ' [' + (c + 1) + '/' + chunks.length + ']' : '';
            var header = titleBase + part + ' — событий: ' + log.length + '\n\n';
            sendToTelegram(header + chunks[c], false, null);
        }

        debugLog('[АВТОШКОЛА] Детальный лог (' + kind + ') отправлен: ' +
                 log.length + ' событий, ' + chunks.length + ' сообщени(й)');
    }
            ...
            if (avto.recording) {
                ...
                if (code !== undefined && !avto.heldKeys.has(code)) {
                    ...
                    debugLog('[АВТОШКОЛА] ▼touch ' +
                             ...
                             ' @ ' + t.toFixed(3) + 'мс | ' + _fmtPos(snap.pos));
                }
            }
