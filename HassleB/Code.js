...
// ==================== HB MENU SYSTEM ====================
// ╔══════════════════════════════════════════════════════════════╗
// ║  ЕДИНАЯ СХЕМА МЕНЮ — единственное место для изменений.      ║
// ║  Добавить кнопку сюда = она появится СРАЗУ везде:           ║
// ║  • в игровом /hb диалоге                                    ║
// ║  • в Telegram inline-кнопках (через showControlsMenu и др.) ║
// ║                                                              ║
// ║  Формат элемента:                                            ║
// ║    label   — текст кнопки (игра поддерживает {COLOR}...)     ║
// ║    next    — имя следующего меню (переход)                   ║
// ║    action  — строка действия (выполнить код)                 ║
// ║    show    — () => bool, если false — кнопка скрыта          ║
// ╚══════════════════════════════════════════════════════════════╝
...
// Обратная карта: имя → dialogId
const HB_MENU_ID = {
    main:             HB_DIALOG_IDS.MAIN,
    controls:         HB_DIALOG_IDS.CONTROLS,
    local_functions:  HB_DIALOG_IDS.LOCAL_FUNCTIONS,
    global_functions: HB_DIALOG_IDS.GLOBAL_FUNCTIONS,
    movement:         HB_DIALOG_IDS.MOVEMENT_CONTROLS,
    afk_modes:        HB_DIALOG_IDS.AFK_MODES,
    afk_pauses:       HB_DIALOG_IDS.AFK_PAUSES,
    afk_reconnect:    HB_DIALOG_IDS.AFK_RECONNECT,
    afk_restart:      HB_DIALOG_IDS.AFK_RESTART,
    pdc_menu:         HB_DIALOG_IDS.PDC_MENU,
};
...
const ARW = '{FFD700}> {FFFFFF}';

// ── Единая схема меню ─────────────────────────────────────────
function getHBMenuSchema() {
    const isPaused          = !!window.getInterfaceStatus('PauseMenu');
    const isAutoLoginOn     = !!autoLoginConfig.enabled;
    const pdcActive         = typeof pdCycle !== 'undefined' && pdCycle.active;

    return {
        main: {
            title: '{00BFFF}Hassle | Bot TG Menu',
            back: null,
            items: [
                { label: `${ARW}Управление`, next: 'controls' }
            ]
        },
        controls: {
            title: '{00BFFF}Управление',
            back: 'main',
            items: [
                { label: `${ARW}Функции`,         next: 'local_functions' },
                { label: `${ARW}Общие функции`,    next: 'global_functions' },
                { label: '{FFFFFF}Инфо об аккаунте', action: 'account_info' },
                {
                    label: `{FFFFFF}Реконнект ${config.autoReconnectEnabled ? ON : OFF}`,
                    action: 'toggle_reconnect',
                    show: () => !!RECONNECT_ENABLED_DEFAULT
                }
            ]
        },
        local_functions: {
            title: '{00BFFF}Функции',
            back: 'controls',
            items: [
                { label: `${ARW}Движение`,                                                              next: 'movement' },
                { label: `{FFFFFF}Увед. правик ${config.govMessagesEnabled ? ON : OFF}`,                action: 'toggle_soob_local' },
                { label: `{FFFFFF}Отслеживание ${config.trackLocationRequests ? ON : OFF}`,             action: 'toggle_mesto_local' },
                { label: `{FFFFFF}Рация все ${config.radioOfficialNotifications ? ON : OFF}`,           action: 'toggle_radio_local' },
                { label: `{FFFFFF}Рация фильтр ${config.radioImportantFilter ? ON : OFF}`,              action: 'toggle_radio_filter_local' },
                { label: `{FFFFFF}Выговоры ${config.warningNotifications ? ON : OFF}`,                  action: 'toggle_warning_local' },
                { label: `{FFFFFF}Написать в чат`,                                                      action: 'request_chat_message' },
                { label: `{FFFFFF}${isPaused ? 'Выйти с паузы' : 'Уйти на паузу'}`,                    action: 'toggle_pause' },
                { label: `{FFFFFF}${isAutoLoginOn ? 'Уйти на автр.' : 'Вернуть автр.'}`,               action: 'toggle_autologin' }
            ]
        },
        global_functions: {
            title: '{00BFFF}Общие функции',
            back: 'controls',
            items: [
                { label: `{FFFFFF}PayDay ${config.paydayNotifications ? ON : OFF}`,                     action: 'toggle_payday' },
                { label: `{FFFFFF}Сообщ. ${config.govMessagesEnabled ? ON : OFF}`,                      action: 'toggle_soob' },
                { label: `{FFFFFF}Место ${config.trackLocationRequests ? ON : OFF}`,                    action: 'toggle_mesto' },
                { label: `{FFFFFF}Рация все ${config.radioOfficialNotifications ? ON : OFF}`,           action: 'toggle_radio' },
                { label: `{FFFFFF}Рация фильтр ${config.radioImportantFilter ? ON : OFF}`,              action: 'toggle_radio_filter' },
                { label: `{FFFFFF}Выговоры ${config.warningNotifications ? ON : OFF}`,                  action: 'toggle_warning' },
                { label: `${ARW}AFK Ночь`,                                                              action: 'afk_night' },
                { label: `${ARW}AFK`,                                                                   action: 'afk_standard' },
                { label: `${ARW}PayDay Цикл ${pdcActive ? ON : OFF}`,                                   next: 'pdc_menu' },
                { label: `${ARW}Прокачка уровня`,                                                       action: 'levelup',
                  show: () => !!config.autoReconnectEnabled }
            ]
        },
        movement: {
            title: '{00BFFF}Движение',
            back: 'local_functions',
            items: [
                { label: '{FFFFFF}^ Вперед',  action: 'move_forward' },
                { label: '{FFFFFF}< Влево',   action: 'move_left'    },
                { label: '{FFFFFF}> Вправо',  action: 'move_right'   },
                { label: '{FFFFFF}v Назад',   action: 'move_back'    },
                { label: '{FFFFFF}Прыжок',    action: 'move_jump'    },
                { label: '{FFFFFF}Удар',      action: 'move_punch'   },
                { label: `{FFFFFF}${config.isSitting ? 'Встать' : 'Сесть'}`, action: 'move_sit_stand' }
            ]
        },
        afk_modes: {
            title: '{00BFFF}AFK Ночь - Режим',
            back: 'global_functions',
            items: [
                { label: `${ARW}С паузами`,  next: 'afk_pauses'                   },
                { label: `${ARW}Без пауз`,   action: 'afk_none'                    }
            ]
        },
        afk_pauses: {
            title: '{00BFFF}AFK Ночь - Паузы',
            back: 'afk_modes',
            items: [
                { label: `${ARW}5/5 минут`,       action: 'afk_fixed'  },
                { label: `${ARW}Рандомное время`,  action: 'afk_random' }
            ]
        },
        afk_reconnect: {
            title: '{00BFFF}AFK Ночь - Реконнект',
            back: 'afk_pauses',
            items: [
                { label: '{00FF00}Реконнект ВКЛ',  action: 'afk_reconnect_on'  },
                { label: '{FF0000}Реконнект ВЫКЛ', action: 'afk_reconnect_off' }
            ]
        },
        afk_restart: {
            title: '{00BFFF}AFK Ночь - Действие',
            back: 'afk_reconnect',
            items: [
                { label: '{FFFFFF}/q',   action: 'afk_restart_q'   },
                { label: '{FFFFFF}/rec', action: 'afk_restart_rec' }
            ]
        },
        pdc_menu: {
            title: '{00BFFF}PayDay Цикл',
            back: 'global_functions',
            items: [
                { label: `${ARW}Запустить`,   action: 'pdc_start' },
                { label: `${ARW}Остановить`,  action: 'pdc_stop'  }
            ]
        }
    };
}

// ── Рендерер: игровой диалог ──────────────────────────────────
function renderHBDialog(menuKey) {
    const schema = getHBMenuSchema();
    const menu   = schema[menuKey];
    if (!menu) return;

    const dialogId = HB_MENU_ID[menuKey];
    if (!dialogId) return;

    // Фильтруем скрытые
    const items = menu.items.filter(i => !i.show || i.show());

    let list = menu.back !== null
        ? '{FFA500}< Назад<n>'   // есть родитель — показываем "Назад"
        : '';                    // главное меню — без "Назад"

    items.forEach(i => { list += `${i.label}<n>`; });

    window.addDialogInQueue(
        `[${dialogId},2,"${menu.title}","","Выбрать","Закрыть",0,0]`,
        list,
        0
    );
}
...
function handleHBMenuSelection(dialogId, button, listitem) {
    ...
    // Найти menuKey по dialogId
    const menuKey = Object.keys(HB_MENU_ID).find(k => HB_MENU_ID[k] === dialogId);
    if (!menuKey) return;
    ...
    const items   = menu.items.filter(i => !i.show || i.show());
    const hasBack = menu.back !== null;

    // listitem 0 = "Назад" (если есть)
    if (hasBack && listitem === 0) {
        setTimeout(() => renderHBDialog(menu.back), 100);
        return;
    }

    // Индекс реального элемента
    const itemIndex = hasBack ? listitem - 1 : listitem;
    const item = items[itemIndex];
    if (!item) return;

    // Переход в подменю
    if (item.next) {
        setTimeout(() => renderHBDialog(item.next), 100);
        return;
    }

    // Выполнить действие
    if (item.action) {
        hbDoAction(item.action, menuKey);
    }
}

// ── Все действия ──────────────────────────────────────────────
function hbDoAction(action, fromMenu) {
    switch (action) {
        ...
        case 'account_info': {
            try {
                ...
                let level = '❓', hours = '❓';
                try {
                    const l = window.App.$store.getters['player/level'];
                    const h = window.App.$store.getters['player/passedHours'];
                    if (l != null) level = l;
                    if (h != null) hours = h;
                } catch(e) {}
                const posStr = pos
                    ...
                    : '❓ Позиция недоступна';
                const cash = moneyData?.money    != null ? `₽${moneyData.money.toLocaleString()}`     : '❓';
                const bank = moneyData?.bankMoney != null ? `₽${moneyData.bankMoney.toLocaleString()}` : '❓';
                sendToTelegram(
                    `📊 <b>Инфо об аккаунте (${displayName})</b>\n\n` +
                    `👤 <b>Ник:</b> ${nick}  |  <b>Сервер:</b> S${server}\n` +
                    `🎭 <b>Скин ID:</b> ${skinId}  ${faction}\n` +
                    `⭐ <b>Уровень:</b> ${level}  |  ⏱ <b>Часов:</b> ${hours}\n\n` +
                    `📍 <b>Позиция:</b>\n<code>${posStr}</code>\n\n` +
                    `💵 <b>Нал:</b> ${cash}\n🏦 <b>Банк:</b> ${bank}\n\n` +
                    `📋 <b>Лог сессии:</b>\n` +
                    (globalState.sessionLog.length > 0
                        ? globalState.sessionLog.slice(-10).join('\n')
                        : '<i>Лог пуст</i>'),
                    false, null
                );
                showScreenNotification('Hassle', 'Инфо отправлено в Telegram');
            } catch(e) {
                sendToTelegram(`❌ <b>Ошибка инфо (${displayName}):</b> ${e.message}`, false, null);
            }
            break;
        }
        ...
        case 'toggle_reconnect':
            ...
            showScreenNotification('Hassle', `Реконнект ${config.autoReconnectEnabled ? 'ВКЛ' : 'ВЫКЛ'}`);
            setTimeout(() => renderHBDialog('controls'), 100);
            break;
        ...
        case 'toggle_pause': {
            const isPaused = !!window.getInterfaceStatus('PauseMenu');
            try {
                isPaused ? closeInterface('PauseMenu') : openInterface('PauseMenu');
                sendToTelegram(`${isPaused ? '▶️' : '⏸️'} <b>${isPaused ? 'Вышли из паузы' : 'Вошли в паузу'} (${displayName})</b>`, true, null);
            } catch(e) { sendToTelegram(`❌ Ошибка паузы: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('local_functions'), 300);
            break;
        }
        ...
        case 'toggle_autologin':
            ...
            sendChatInput('/rec 5');
            sendToTelegram(`${autoLoginConfig.enabled ? '✅' : '🚫'} <b>Автовход ${autoLoginConfig.enabled ? 'включён' : 'отключён'}, отправлен /rec 5 (${displayName})</b>`, false, null);
            setTimeout(() => renderHBDialog('local_functions'), 100);
            break;

        // ── Написать в чат ────────────────────────────────────
        case 'request_chat_message': {
            const pendingKey = `${config.chatIds[0]}_${uniqueId}`;
            pendingInputs[pendingKey] = { type: 'chat_message', timestamp: Date.now() };
            config.chatIds.forEach(cid => {
                sendToTelegram(`✉️ Введите сообщение для ${displayName}:\n🔑 ID: ${uniqueId}`, false, null);
            });
            showScreenNotification('Hassle', 'Ожидаю сообщение в Telegram');
            break;
        }

        // ── Локальные переключатели ───────────────────────────
        case 'toggle_soob_local':
            config.govMessagesEnabled = !config.govMessagesEnabled;
            showScreenNotification('Hassle', `Увед. правик ${config.govMessagesEnabled ? 'ВКЛ' : 'ВЫКЛ'}`);
            setTimeout(() => renderHBDialog('local_functions'), 100);
            break;
        case 'toggle_mesto_local':
            config.trackLocationRequests = !config.trackLocationRequests;
            showScreenNotification('Hassle', `Отслеживание ${config.trackLocationRequests ? 'ВКЛ' : 'ВЫКЛ'}`);
            setTimeout(() => renderHBDialog('local_functions'), 100);
            break;
        case 'toggle_radio_local':
            config.radioOfficialNotifications = !config.radioOfficialNotifications;
            showScreenNotification('Hassle', `Рация все ${config.radioOfficialNotifications ? 'ВКЛ' : 'ВЫКЛ'}`);
            setTimeout(() => renderHBDialog('local_functions'), 100);
            break;
        case 'toggle_radio_filter_local':
            config.radioImportantFilter = !config.radioImportantFilter;
            showScreenNotification('Hassle', `Рация фильтр ${config.radioImportantFilter ? 'ВКЛ' : 'ВЫКЛ'}`);
            setTimeout(() => renderHBDialog('local_functions'), 100);
            break;
        case 'toggle_warning_local':
            config.warningNotifications = !config.warningNotifications;
            showScreenNotification('Hassle', `Выговоры ${config.warningNotifications ? 'ВКЛ' : 'ВЫКЛ'}`);
            setTimeout(() => renderHBDialog('local_functions'), 100);
            break;

        // ── Глобальные переключатели ──────────────────────────
        case 'toggle_payday':
            broadcastGlobalCommand('toggle_payday', config.paydayNotifications ? 'off' : 'on');
            setTimeout(() => renderHBDialog('global_functions'), 100);
            break;
        case 'toggle_soob':
            broadcastGlobalCommand('toggle_soob', config.govMessagesEnabled ? 'off' : 'on');
            setTimeout(() => renderHBDialog('global_functions'), 100);
            break;
        case 'toggle_mesto':
            broadcastGlobalCommand('toggle_mesto', config.trackLocationRequests ? 'off' : 'on');
            setTimeout(() => renderHBDialog('global_functions'), 100);
            break;
        case 'toggle_radio':
            broadcastGlobalCommand('toggle_radio', config.radioOfficialNotifications ? 'off' : 'on');
            setTimeout(() => renderHBDialog('global_functions'), 100);
            break;
        case 'toggle_radio_filter':
            broadcastGlobalCommand('toggle_radio_filter', config.radioImportantFilter ? 'off' : 'on');
            setTimeout(() => renderHBDialog('global_functions'), 100);
            break;
        case 'toggle_warning':
            broadcastGlobalCommand('toggle_warning', config.warningNotifications ? 'off' : 'on');
            setTimeout(() => renderHBDialog('global_functions'), 100);
            break;

        // ── AFK ───────────────────────────────────────────────
        case 'afk_night':
            setTimeout(() => renderHBDialog('afk_modes'), 100);
            break;
        case 'afk_standard':
            if (config.autoReconnectEnabled) {
                currentHBSelectedMode = 'none';
                setTimeout(() => renderHBDialog('afk_reconnect'), 100);
            } else {
        ...
        case 'afk_random':
            ...
            break;
        case 'afk_reconnect_on':
            setTimeout(() => renderHBDialog('afk_restart'), 100);
            break;
        case 'afk_reconnect_off':
            activateAFKWithMode(currentHBSelectedMode, false, 'q', null, null);
            showScreenNotification('Hassle', 'AFK активирован (реконнект выкл)');
            currentHBSelectedMode = null;
            break;
        case 'afk_restart_q':
            activateAFKWithMode(currentHBSelectedMode, true, 'q', null, null);
            showScreenNotification('Hassle', 'AFK активирован (/q при рестарте)');
            currentHBSelectedMode = null;
            break;
        case 'afk_restart_rec':
            activateAFKWithMode(currentHBSelectedMode, true, 'rec', null, null);
            showScreenNotification('Hassle', 'AFK активирован (/rec при рестарте)');
            currentHBSelectedMode = null;
            break;
        ...
        // ── PDC ───────────────────────────────────────────────
        case 'pdc_start':
            try { pdcStart(); showScreenNotification('Hassle', 'PayDay цикл запущен'); } catch(e) {}
            setTimeout(() => renderHBDialog('pdc_menu'), 300);
            break;
        case 'pdc_stop':
            try { pdcStop(); showScreenNotification('Hassle', 'PayDay цикл остановлен'); } catch(e) {}
            setTimeout(() => renderHBDialog('pdc_menu'), 300);
            break;
        ...
        case 'move_forward':
            try {
                ...
                sendToTelegram(`🚶 <b>Вперед (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;
        case 'move_back':
            try {
                window.onScreenControlTouchStart('<Gamepad>/leftStick');
                ...
                sendToTelegram(`🚶 <b>Назад (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;
        case 'move_left':
            try {
                ...
                sendToTelegram(`🚶 <b>Влево (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;
        case 'move_right':
            try {
                ...
                sendToTelegram(`🚶 <b>Вправо (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;
        case 'move_jump':
            try {
                ...
                sendToTelegram(`🆙 <b>Прыжок (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;
        case 'move_punch':
            try {
                ...
                sendToTelegram(`👊 <b>Удар (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;
        case 'move_sit_stand':
            try {
                ...
                const txt = config.isSitting ? 'Сесть' : 'Встать';
                showScreenNotification('Hassle', txt);
                sendToTelegram(`✅ <b>${txt} (${displayName})</b>`, false, null);
            } catch(e) { sendToTelegram(`❌ Ошибка: ${e.message}`, false, null); }
            setTimeout(() => renderHBDialog('movement'), 100);
            break;

        default:
            debugLog(`[HB] Неизвестное действие: ${action}`);
    }
}
