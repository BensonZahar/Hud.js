...
// ==================== DIALOG WINDOW TELEGRAM CONTROL MODULE ====================
// Этот модуль перехватывает диалоговые окна и отправляет их в Telegram с inline-кнопками
...
function dialogLog(message, data = null) {
    if (DIALOG_MODULE_CONFIG.debugMode) {
        const timestamp = new Date().toLocaleTimeString();
        console.log(`[${timestamp}] [DIALOG MODULE]`, message, data || '');
    }
}
...
function createDialogKeyboard(dialogData) {
    const { dialogID, dialogType, buttons, items, paginate } = dialogData;
    const keyboard = [];
    ...
    if (dialogType.includes('list') && items && items.length > 0) {
        ...
        for (let i = 0; i < maxButtons; i++) {
            buttonsRow.push({
                text: `${i + 1}`,
                callback_data: `dialog_select_${dialogID}_${i}`
            });
        }
        ...
        if (paginate && (paginate[0] || paginate[1])) {
            ...
            if (paginate[0]) {
                paginateRow.push({
                    text: '◀️ Назад',
                    callback_data: `dialog_paginate_${dialogID}_0`
                });
            }
            if (paginate[1]) {
                paginateRow.push({
                    text: 'Вперёд ▶️',
                    callback_data: `dialog_paginate_${dialogID}_1`
                });
            }
            keyboard.push(paginateRow);
        }
    }
    
    // Добавляем основные кнопки диалога
    const mainButtons = [];
    
    if (buttons[0] && buttons[0] !== '') {
        mainButtons.push({
            text: `✅ ${cleanDialogText(buttons[0])}`,
            callback_data: `dialog_button_${dialogID}_1`
        });
    }
    
    if (buttons[1] && buttons[1] !== '') {
        mainButtons.push({
            text: `❌ ${cleanDialogText(buttons[1])}`,
            callback_data: `dialog_button_${dialogID}_0`
        });
    }
...
function sendDialogToTelegram(dialogData) {
    if (!DIALOG_MODULE_CONFIG.sendToTelegram || !config.botToken) {
        dialogLog('Отправка в Telegram отключена или нет токена');
        return;
    }
    ...
    if (title) {
        message += `<b>📋 ${formatForTelegram(cleanDialogText(title))}</b>\n`;
    }
    ...
    config.chatIds.forEach(chatId => {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                dialogData.telegramMessageIds.push({
                    chatId: chatId,
...
function deleteDialogFromTelegram(dialogData) {
    ...
    dialogData.telegramMessageIds.forEach(({ chatId, messageId }) => {
        deleteMessage(chatId, messageId);
    });
...
window.addDialogInQueue = function(openParams, stringParam, priority) {
    try {
        ...
        const dialogType = DIALOG_TYPES[typeIndex] || `unknown_${typeIndex}`;
...
window.handleDialogModuleCommands = function(message, chatId) {
    ...
    if (message === '/dialog_status' || message.startsWith('/dialog_status ')) {
        ...
        let statusMessage = `📊 <b>Статус модуля диалогов (${displayName})</b>\n\n`;
        statusMessage += `Состояние: ${status}\n`;
        statusMessage += `Активных диалогов: ${activeCount}\n`;
        statusMessage += `Автоответ: ${DIALOG_MODULE_CONFIG.autoResponseEnabled ? '🟢 Вкл' : '🔴 Выкл'}\n`;
        
        if (activeCount > 0) {
            statusMessage += '\n<b>Активные диалоги:</b>\n';
            activeDialogs.forEach((data, id) => {
                statusMessage += `• ID ${id}: ${cleanDialogText(data.title).substring(0, 30)}\n`;
            });
        }
        
        sendToTelegram(statusMessage, false, null);
        return true;
    }
