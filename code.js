...
(function() {
    ...
    // КОНФИГУРАЦИЯ
    const userConfig = {
        userNumber: "6", // Пример: "3"
        botToken: '8184449811:AAE-nssyxdjAGnCkNCKTMN8rc2xgWEaVOFA', // Замените на ваш токен
        chatIds: ['1046461621'], // Замените на ваши ID чатов
        keywords: [],
        clearDelay: 3000,
        maxAttempts: 15,
        checkInterval: 1000,
    ...
    // Создаем отображаемое имя с ником и сервером для уведомлений
        ...
        tt.methods.add = function(e, i, t) {
            ...
            if (!isNonRPMessage(msg) && checkRoleAndActionConditions(lowerCaseMessage)) {
                ...
                if (foundIDs.length > 0) {
                    ...
                    sendToTelegram(`🔍 <b>Обнаружено условие (${displayName}):</b>\nРоль + действие + ID: ${foundIDs.join(', ')}\n<code>${msg.replace(/</g, '<')}</code>`);
                }
            }
