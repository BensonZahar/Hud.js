...
const config = {
    ...
    podbrosCounter: 0,
    initialized: false,
    accountInfo: { nickname: null, server: null },
    lastPlayerId: null,
    govMessageTrackers: {},
    isSitting: false,
    afkCycle: {
...
function debugLog(message) {
    if (config.debug) {
        console.log(`[DEBUG][${config.accountInfo.nickname || 'Unknown'}]`, message);
    }
}
...
function sendWelcomeMessage() {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("⚙️ Управление", `show_controls_${config.accountInfo.nickname}`)]
        ]
    };
...
function showControlsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("⚙️ Функции", `show_local_functions_${config.accountInfo.nickname}`)],
            [createButton("📋 Общие функции", "show_global_functions")],
            [createButton("⬆️ Свернуть", `hide_controls_${config.accountInfo.nickname}`)]
        ]
    };
...
function showGlobalFunctionsMenu(chatId, messageId) {
    const replyMarkup = {
        inline_keyboard: [
            ...
            [createButton("⬅️ Назад", `show_controls_${config.accountInfo.nickname}`)]
        ]
    };
...
function showLocalFunctionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [createButton("🚶 Движение", `show_movement_controls_${config.accountInfo.nickname}`)],
            [createButton("🏛️ Увед. правик", `show_local_soob_options_${config.accountInfo.nickname}`)],
            [createButton("📍 Отслеживание", `show_local_mesto_options_${config.accountInfo.nickname}`)],
            [createButton("📝 Написать в чат", `request_chat_message_${config.accountInfo.nickname}`)],
            [createButton("⬅️ Назад", `show_controls_${config.accountInfo.nickname}`)]
        ]
    };
...
function showMovementControlsMenu(chatId, messageId, isNotification = false) {
    ...
    const backButton = isNotification ? [] : [[createButton("⬆️ Свернуть", `show_local_functions_${config.accountInfo.nickname}`)]];
    const sitStandButton = config.isSitting
        ? createButton("🧍 Встать", `move_stand_${config.accountInfo.nickname}`)
        : createButton("🪑 Сесть", `move_sit_${config.accountInfo.nickname}`);
    const replyMarkup = {
        inline_keyboard: [
            [createButton("⬆️ Вперед", `move_forward_${config.accountInfo.nickname}`)],
            [createButton("⬅️ Влево", `move_left_${config.accountInfo.nickname}`), createButton("➡️ Вправо", `move_right_${config.accountInfo.nickname}`)],
            [createButton("⬇️ Назад", `move_back_${config.accountInfo.nickname}`)],
            [createButton("🆙 Прыжок", `move_jump_${config.accountInfo.nickname}`)],
            [sitStandButton],
            ...backButton
        ]
    };
...
function showLocalSoobOptionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [
                createButton("🔔 ВКЛ", `local_soob_on_${config.accountInfo.nickname}`),
                createButton("🔕 ВЫКЛ", `local_soob_off_${config.accountInfo.nickname}`)
            ],
            [createButton("⬅️ Назад", `show_local_functions_${config.accountInfo.nickname}`)]
        ]
    };
...
function showLocalMestoOptionsMenu(chatId, messageId) {
    ...
    const replyMarkup = {
        inline_keyboard: [
            [
                createButton("🔔 ВКЛ", `local_mesto_on_${config.accountInfo.nickname}`),
                createButton("🔕 ВЫКЛ", `local_mesto_off_${config.accountInfo.nickname}`)
            ],
            [createButton("⬅️ Назад", `show_local_functions_${config.accountInfo.nickname}`)]
        ]
    };
...
function processUpdates(updates) {
    for (const update of updates) {
        ...
        if (update.message) {
            ...
            else if (message.startsWith(`/chat${config.accountInfo.nickname} `)) {
                const textToSend = message.replace(`/chat${config.accountInfo.nickname} `, '').trim();
            ...
            const isGlobalCommand = message.startsWith('global_') || 
                                    ...
                                    message === 'show_global_functions';

            let callbackNickname = null;
            if (message.startsWith('show_controls_')) {
                callbackNickname = message.replace('show_controls_', '');
            } else if (message.startsWith('show_local_functions_')) {
                callbackNickname = message.replace('show_local_functions_', '');
            } else if (message.startsWith('show_movement_controls_')) {
                callbackNickname = message.replace('show_movement_controls_', '');
            } else if (message.startsWith('show_movement_')) {
                callbackNickname = message.replace('show_movement_', '');
            } else if (message.startsWith('hide_controls_')) {
                callbackNickname = message.replace('hide_controls_', '');
            } else if (message.startsWith('request_chat_message_')) {
                callbackNickname = message.replace('request_chat_message_', '');
            } else if (message.startsWith('local_soob_on_')) {
                callbackNickname = message.replace('local_soob_on_', '');
            } else if (message.startsWith('local_soob_off_')) {
                callbackNickname = message.replace('local_soob_off_', '');
            } else if (message.startsWith('local_mesto_on_')) {
                callbackNickname = message.replace('local_mesto_on_', '');
            } else if (message.startsWith('local_mesto_off_')) {
                callbackNickname = message.replace('local_mesto_off_', '');
            } else if (message.startsWith('move_forward_')) {
                callbackNickname = message.replace('move_forward_', '');
            } else if (message.startsWith('move_back_')) {
                callbackNickname = message.replace('move_back_', '');
            } else if (message.startsWith('move_left_')) {
                callbackNickname = message.replace('move_left_', '');
            } else if (message.startsWith('move_right_')) {
                callbackNickname = message.replace('move_right_', '');
            } else if (message.startsWith('move_jump_')) {
                callbackNickname = message.replace('move_jump_', '');
            } else if (message.startsWith('move_sit_')) {
                callbackNickname = message.replace('move_sit_', '');
            } else if (message.startsWith('move_stand_')) {
                callbackNickname = message.replace('move_stand_', '');
            } else if (message.startsWith('admin_reply_')) {
                callbackNickname = message.replace('admin_reply_', '');
            } else if (message.startsWith('show_local_soob_options_')) {
                callbackNickname = message.replace('show_local_soob_options_', '');
            } else if (message.startsWith('show_local_mesto_options_')) {
                callbackNickname = message.replace('show_local_mesto_options_', '');
            }

            // Проверяем, является ли команда локальной (только для текущего аккаунта)
            const isForThisBot = isGlobalCommand || 
                                 (callbackNickname && callbackNickname === config.accountInfo.nickname) ||
            ...
            // Обработка команд
            if (message === `show_controls_${config.accountInfo.nickname}`) {
                showControlsMenu(chatId, messageId);
            } 
            else if (message === "show_global_functions") {
                showGlobalFunctionsMenu(chatId, messageId);
            }
            else if (message === `show_local_functions_${config.accountInfo.nickname}`) {
                showLocalFunctionsMenu(chatId, messageId);
            }
            else if (message === `show_movement_controls_${config.accountInfo.nickname}`) {
                showMovementControlsMenu(chatId, messageId);
            }
            else if (message.startsWith("show_movement_")) {
                const accountNickname = message.replace("show_movement_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    showMovementControlsMenu(chatId, messageId, true);
                }
            }
            else if (message === `hide_controls_${config.accountInfo.nickname}`) {
                hideControlsMenu(chatId, messageId);
            }
            else if (message === `request_chat_message_${config.accountInfo.nickname}`) {
                const requestMsg = `✉️ Введите сообщение для ${displayName}:\n(Будет отправлено как /chat${config.accountInfo.nickname} ваш_текст)`;
                ...
                const accountNickname = message.replace("admin_reply_", "");
                if (accountNickname === config.accountInfo.nickname) {
                ...
                const accountNickname = message.replace("move_forward_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                ...
                const accountNickname = message.replace("move_back_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                        window.onScreenControlTouchStart("<Gamepad>/leftStick");
                        window.onScreenControlTouchMove("<Gamepad>/leftStick", 0, -1);
                ...
                const accountNickname = message.replace("move_left_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                        window.onScreenControlTouchStart("<Gamepad>/leftStick");
                ...
                const accountNickname = message.replace("move_right_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                        window.onScreenControlTouchStart("<Gamepad>/leftStick");
                        window.onScreenControlTouchMove("<Gamepad>/leftStick", 1, 0);
                ...
                const accountNickname = message.replace("move_jump_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                ...
                const accountNickname = message.replace("move_sit_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                ...
                const accountNickname = message.replace("move_stand_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    try {
                        window.onScreenControlTouchStart("<Keyboard>/c");
                        setTimeout(() => window.onScreenControlTouchEnd("<Keyboard>/c"), 500);
                ...
                const accountNickname = message.replace("show_local_soob_options_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    showLocalSoobOptionsMenu(chatId, messageId);
                }
            }
            else if (message.startsWith("show_local_mesto_options_")) {
                const accountNickname = message.replace("show_local_mesto_options_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    showLocalMestoOptionsMenu(chatId, messageId);
                }
            }
            else if (message.startsWith("local_soob_on_")) {
                const accountNickname = message.replace("local_soob_on_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    config.govMessagesEnabled = true;
                    sendToTelegram(`🔔 <b>Уведомления от сотрудников правительства включены для ${displayName}</b>`, false, null, config.notificationDeleteDelay);
                    sendWelcomeMessage();
                ...
                const accountNickname = message.replace("local_soob_off_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    config.govMessagesEnabled = false;
                    sendToTelegram(`🔕 <b>Уведомления от сотрудников правительства отключены для ${displayName}</b>`, false, null, config.notificationDeleteDelay);
                    sendWelcomeMessage();
                ...
                const accountNickname = message.replace("local_mesto_on_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    config.trackLocationRequests = true;
                    sendToTelegram(`📍 <b>Отслеживание запросов местоположения включено для ${displayName}</b>`, false, null, config.notificationDeleteDelay);
                    sendWelcomeMessage();
                ...
                const accountNickname = message.replace("local_mesto_off_", "");
                if (accountNickname === config.accountInfo.nickname) {
                    config.trackLocationRequests = false;
                    sendToTelegram(`🔕 <b>Отслеживание запросов местоположения отключено для ${displayName}</b>`, false, null, config.notificationDeleteDelay);
                    sendWelcomeMessage();
...
function handlePayDayTimeMessage() {
    ...
    const mainTimerDuration = 59 * 60 * 1000 + 55 * 1000;
    ...
    window.OnChatAddMessage = function(e, i, t) {
        ...
        if (lowerCaseMessage.includes("зареспавнил вас")) {
            ...
            const replyMarkup = {
                inline_keyboard: [
                    [
                        createButton("📝 Ответить", `admin_reply_${config.accountInfo.nickname}`),
                        createButton("🚶 Движения", `show_movement_${config.accountInfo.nickname}`)
                    ]
                ]
            };
        ...
        if ((lowerCaseMessage.indexOf("администратор") !== -1 && lowerCaseMessage.indexOf("для") !== -1) ||
            ...
            if (lowerCaseMessage.includes("подбросил")) {
                ...
                const replyMarkup = {
                    inline_keyboard: [
                        [
                            createButton("📝 Ответить администратору", `admin_reply_${config.accountInfo.nickname}`),
                            createButton("🚶 Движения", `show_movement_${config.accountInfo.nickname}`)
                        ]
                    ]
                };
