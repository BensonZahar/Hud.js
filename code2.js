...
(function() {
    ...
    // КОНФИГУРАЦИЯ
    const userConfig = {
        userNumber: "1", // Текущий номер аккаунта (изменять для каждого пользователя)
        botToken: '7203337635:AAHgE-jr8nkdFTk2Y-PlH214d4drF-WRLUk',
        chatIds: ['5515408606', 'ВТОРОЙ_CHAT_ID'],
        keywords: [],
        clearDelay: 3000,
        maxAttempts: 15,
        checkInterval: 1000,
    ...
    function initializeChatMonitor() {
        if (typeof je === 'undefined' || typeof Je.methods === 'undefined' || typeof Je.methods.add !== 'function') {
            debugLog('je.methods.add еще не доступен');
            return false;
        }
        ...
        const originalAdd = Je.methods.add;

        Je.methods.add = function(e, i, t) {
