...
class MEmuHudManager:
    ...
    # Загружаем List.js для получения списка пользователей
    try:
        ...
        for user in users:
            self.code_files.append({
                'name': f'{user}.js',  # Для совместимости с остальным кодом
                'url': None,  # URL не нужен, т.к. используется Load.js
                'html_url': None,
                'user': user  # Добавляем имя пользователя
            })
                    ...
                    if callback_data.startswith("code_"):
                        try:
                            ...
                            if 0 <= index < len(self.code_files):
                                ...
                                selected_user = self.code_files[index].get('user')
                                self.selected_code_name = selected_user  # Сохраняем только имя пользователя
                                self.selected_code_url = None  # URL не используется
                                
                                # Получаем информацию о последнем коммите Load.js
                                self.last_commit_info = self.fetch_last_commit("Load.js", "HassleB")
                                
                                if not self.full_logging:
                                    self.root.after(0, lambda: self.update_waiting_message("Пользователь выбран. Ожидание выбора режима отладки..."))
                                else:
                                    self.root.after(0, lambda: self.update_waiting_message(f"Выбран пользователь: {selected_user}. Ожидание выбора режима отладки..."))
                            ...
                            self.log("[X] Ошибка: Ошибка обработки выбора пользователя")
