...
class MEmuHudManager:
    ...
    def setup_gui(self):
        ...
        conn_menu = ctk.CTkComboBox(self.main_frame,
                                   values=["1 - Физическое устройство", "2 - Клонированное хранилище (999)", "3 - Эмулятор Nox"],
    ...
    def check_nox_installation(self):
        ...
        for path in self.nox_paths:
            if Path(path).exists():
                self.nox_path = path
                self.nox_adb = path.replace("Nox.exe", "adb.exe")
                if not self.full_logging:
    ...
    def select_connection(self):
        ...
        if not self.local_adb.exists() and not self.nox_adb:
