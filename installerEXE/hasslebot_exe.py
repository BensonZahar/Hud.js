...
class MEmuHudManager:
        ...
        for old_pkg, new_pkg in packages:
            ...
            try:
                ...
                if "exists" in result_new.stdout:
                    ...
                    is_nox = "nox" in self.adb_path.lower() or self.nox_path
                    if is_nox:
                        self.log(f"Копирование (безопасно для Nox) {old_pkg} → {new_pkg}...")
                        cp_cmd = [self.adb_path] + self.device_param + ["shell", "cp", "-a", old_data_path, new_data_path]
                        rm_cmd = [self.adb_path] + self.device_param + ["shell", "rm", "-rf", old_data_path]
                        subprocess.run(cp_cmd, capture_output=True, text=True,
                                      creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0)
                        subprocess.run(rm_cmd, capture_output=True,
                                      creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0)
                        if subprocess.run([self.adb_path] + self.device_param + ["shell", "test", "-d", new_data_path, "&&", "echo", "ok"]).stdout.strip() == "ok":
    ...
    def insert_code_after_mod(self):
        """Переименование папок обратно + вписывание кода — с защитой от багов Nox"""
        if not self.select_connection():
            self.log("[X] Устройство не подключено")
            return

        # Определяем, это Nox или нет
        is_nox = "nox" in self.adb_path.lower() or self.nox_path

        if is_nox:
            self.log("[!] Обнаружен Nox — используем безопасный метод переноса (без mv)")
        ...
        for old_pkg, new_pkg in packages:
            ...
            try:
                ...
                result = subprocess.run(
                    [self.adb_path] + self.device_param + ["shell", "test", "-d", old_data_path, "&&", "echo", "exists"],
                    capture_output=True, text=True,
                    creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0
                )
                ...
                result_new = subprocess.run(
                    [self.adb_path] + self.device_param + ["shell", "test", "-d", new_data_path, "&&", "echo", "exists"],
                    capture_output=True, text=True,
                    creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0
                )

                if "exists" in result_new.stdout:
                    self.log(f"[!] Папка {new_pkg} уже существует — пропускаем")
                    continue

                if is_nox:
                    # БЕЗОПАСНЫЙ СПОСОБ ДЛЯ NOX: копируем, потом удаляем старую
                    self.log(f"Копирование {old_pkg} → {new_pkg} (безопасно для Nox)...")
                    cp_cmd = [self.adb_path] + self.device_param + ["shell", "cp", "-a", old_data_path, new_data_path]
                    cp_result = subprocess.run(cp_cmd, capture_output=True, text=True,
                                              creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0)
                    if cp_result.returncode != 0:
                        self.log(f"[X] Ошибка копирования: {cp_result.stderr.strip()}")
                        continue

                    # Удаляем старую только после успешного копирования
                    rm_cmd = [self.adb_path] + self.device_param + ["shell", "rm", "-rf", old_data_path]
                    subprocess.run(rm_cmd, capture_output=True,
                                  creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0)

                else:
                    # Обычный способ для MEmu и физических устройств
                    self.log(f"Переименование {old_pkg} → {new_pkg}...")
                    mv_cmd = [self.adb_path] + self.device_param + ["shell", "mv", old_data_path, new_data_path]
                    mv_result = subprocess.run(mv_cmd, capture_output=True, text=True,
                                              creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0)
                    if mv_result.returncode != 0:
                        self.log(f"[X] Ошибка переименования: {mv_result.stderr.strip()}")
                        continue

                self.log(f"[√] Папка восстановлена: {new_pkg}")
                success_count += 1

            except Exception as e:
                self.log(f"[X] Ошибка при обработке {old_pkg}: {e}")

        # В любом случае — вписываем код в обе папки
        if success_count > 0 or not is_nox:  # На Nox всё равно пробуем вписать
            self.replace_with_code('com.hassle.online')
            if success_count > 0 or not is_nox:
        ...
        if is_nox:
            self.log("[√] Безопасный перенос для Nox завершён! Кэш сохранён.")
            messagebox.showinfo("Готово!", "Кэш успешно перенесён и защищён от исчезновения на Nox!\n\nМожешь запускать игру — всё будет работать.")
        else:
            self.log("[√] Перенос завершён")
