...
// ══════════════════════════════════════════════════════════════════
//  render — корневой элемент <Modal>, как в Window.js / SideMenu.js.
//  ВАЖНО: слот помечен _:2 (DYNAMIC). Ранее стоял _:1 (STABLE), из-за
//  чего Vue диффил keyed-списки законов по устаревшему кэшу и бросал
//  RangeError: Invalid array length после загрузки документов.
// ══════════════════════════════════════════════════════════════════
function render(_ctx,_cache,$props,$setup,$data,$options){
const currentTabKey=$options.visibleTabs[$data.currentTab]?.key;
const safeDocs=Array.isArray($options.filteredLawDocuments)?$options.filteredLawDocuments:[];
...
:(openBlock(true),createElementBlock(Fragment,null,renderList(safeDocs,(doc)=>(
...
(openBlock(true),createElementBlock(Fragment,null,renderList(Array.isArray(doc.articles)?doc.articles:[],(art)=>(
...
_:2   // ← DYNAMIC-слот (было _:1 — причина RangeError: Invalid array length)
},8,["isOpened","title"]);
}
...
if(!doc||typeof doc!=="object")throw new Error("Bad JSON for "+meta.id);
return doc;
}));
...
// Исправлено: функция объявлена под тем же именем, под которым вызывается
// (ранее была xhrGetLaws, а вызовы шли к _xhrGetLaws → ReferenceError в ретраях).
...
if(this.mode==="laws")return tabs.filter(t=>t.key==="laws");
return tabs;
},
filteredLawDocuments(){
const q=normalizeText(this.search.trim());
let docs=Array.isArray(this.lawDocuments)?this.lawDocuments:[];
...
for(const a of (Array.isArray(doc.articles)?doc.articles:[])){arr.push({...a,docId:doc.id,docTitle:doc.title});}
}
return arr;
},
selectedLawArticle(){
...
const found=(Array.isArray(doc.articles)?doc.articles:[]).find(a=>a.id===baseId);
if(found){
...
for(const doc of this.lawDocuments){
const found=(Array.isArray(doc.articles)?doc.articles:[]).find(a=>a.id===this.selectedLawArticleId);
