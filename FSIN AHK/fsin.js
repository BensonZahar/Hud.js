        ...
        subItems: [
            ...
            },
            {
                ...
                messages: [
                    ...
                    '/s Если гражданин нарушает, попросите отойти на 30 метров. Не подчиняется, досчитайте до 10 и применяйте силу. Бить и стрелять без причины, увольнение и ЧС ФСИН.',
                    '/s Лекция окончена.'
                ]
            },
            {
...
/* ============================================================
   TaxiTimer — отдельный интерфейс (таймер подачи такси)
   ============================================================ */
window.App.$.appContext.components.TaxiTimer = f(() => d(() => import("./TaxiTimer.js"), ["./TaxiTimer.css"], import.meta.url));
window.App.components.TaxiTimer = { open: { status: !1 }, show: !0, options: { hud: !0, style: "z-index: 2" } };

/* ─── Вспомогательная функция: управляет только обводкой радара ─────────────
   Не использует hud.showTaxiEvent() — тот ставит свой таймер и показывает
   дубликат виджета внутри HUD.
   Вместо этого напрямую пишем в реактивный data() Vue 3 — Proxy подхватит. */
...
/* открыть/перезапустить: openTaxiTimer(секунды, текст, вариант)
   вариант 1 = жёлтый (activity), 0 = danger (красный)              */
window.openTaxiTimer = (e = 254, t = "Время подачи", o = 1) => {
    /* 1) Включаем обводку радара (жёлтую или красную) */
    _setTaxiRadarBorder(true, o);

    /* 2) Открываем / перезапускаем сам таймер */
    if (window.getInterfaceStatus("TaxiTimer")) {
        const n = window.interface("TaxiTimer");
        n && n.start(e, t, o);
    } else {
        window.openInterface("TaxiTimer", JSON.stringify([e, t, o]));
    }
};

window.hideTaxiTimer = () => {
    /* Гасим обводку радара */
    _setTaxiRadarBorder(false);
    window.closeInterface("TaxiTimer");
};

/* ---- /tt — перехват на уровне engine.trigger ─────────────────────────────
   Hud.js вызывает engine.trigger("SendChatInput", msg) напрямую,
   не через window.sendChatInput, поэтому перехватываем engine.trigger.
        ...
        if (name === 'SendChatInput') {
            ...
            if (/^\/tt(\s|$)/i.test(msg)) {
                ...
                if (args.length && /^(stop|off|hide)$/i.test(args[0])) {
                    window.hideTaxiTimer && window.hideTaxiTimer();
                } else if (!args.length) {
                    window.openTaxiTimer && window.openTaxiTimer();
                } else {
                    ...
                    window.openTaxiTimer && window.openTaxiTimer(dur, text, v);
                }
